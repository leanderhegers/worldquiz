...
function handleFlagInput(val){
  ...
  _fMatches=Object.keys(C).map(Number).filter(id=>pool(id)&&(cap?CAPITALS[id]:true)&&nameOf(id).toLowerCase().includes(v))
    .sort((a,b)=>{
...
function submitFlag(){
  ...
  if(val===tg.de.toLowerCase()||val===tg.en.toLowerCase()){
