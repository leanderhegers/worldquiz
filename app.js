...
// ── TRIVIA QUIZ ──
// One quiz mixing population-rank and area-rank questions together — reuses the plain country
// quiz's click handler and colouring entirely unchanged: the target of each round is a real
// country id, so id===game.current (handleClick) and the active/found/dim colouring (updateColors'
// final branch, since triviaMode isn't lake/river/city/pin) already do the right thing. The only
// things this mode adds are how the round queue is built and what question text is shown instead
...
const TRIVIA_ROUNDS_PER_SIDE=10;
...
function _areaQuestionText(rank,dir){
  if(rank===1)return dir==='most'?t('areaMostQ'):t('areaLeastQ');
  return dir==='most'?t('areaMostRankQ')(rank):t('areaLeastRankQ')(rank);
}
// Picks up to n countries from one end of a fully-sorted id list, skipping any id already claimed
// by the other category. Population and area rankings overlap heavily at the extremes (Russia/
// China/USA/Brazil/India are all top-10 in both) — without this, a merged round could end up
// asking two different questions about the same country, which breaks nextCountry()'s "skip this
// queue entry, it's already found" check (that check keys on bare country id, with no concept of
// "found for which question"). Rank numbers stay true to the full sorted list (i+1), not to the
// count of successful picks, so a skipped country doesn't shift every later label's claimed rank.
function _triviaPickSide(sorted,dir,n,claimed,questionTextFn){
  const picks=[];
  for(let i=0;i<sorted.length&&picks.length<n;i++){
    const id=dir==='most'?sorted[i]:sorted[sorted.length-1-i];
    if(claimed.has(id))continue;
    claimed.add(id);
    picks.push({id,q:questionTextFn(i+1,dir)});
  }
  return picks;
}
async function startTriviaGame(){
  ...
  const popSorted=Object.keys(C).map(Number).filter(id=>pop[id]!=null).sort((a,b)=>pop[b]-pop[a]);
  const areaSorted=Object.keys(C).map(Number).filter(id=>area[id]!=null).sort((a,b)=>area[b]-area[a]);
  const claimed=new Set();
  const rounds=[
    ..._triviaPickSide(popSorted,'most',TRIVIA_ROUNDS_PER_SIDE,claimed,_popQuestionText),
    ..._triviaPickSide(popSorted,'least',TRIVIA_ROUNDS_PER_SIDE,claimed,_popQuestionText),
    ..._triviaPickSide(areaSorted,'most',TRIVIA_ROUNDS_PER_SIDE,claimed,_areaQuestionText),
    ..._triviaPickSide(areaSorted,'least',TRIVIA_ROUNDS_PER_SIDE,claimed,_areaQuestionText)
  ];
  const shuffled=shuffle(rounds);
  const triviaQuestions={};
  shuffled.forEach(r=>{triviaQuestions[r.id]=r.q;});
  game={mode:'trivia',triviaMode:true,triviaQuestions,
