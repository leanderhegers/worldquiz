...
function gatherSettings(){return {lang,theme,colorblindMode,keepFound,showWrongHint,showSkipHint,showDropdown,projection,learnMode,allTargets,quizRoundLimit,customConts:[...customConts]};}
...
function _triviaBuildSpecs(sorted,dir,claimed,singleQ,multiQ,singleCount,groupN){
  const specs=[];
  let picked=0;
  for(let i=0;i<sorted.length&&picked<singleCount;i++){
...
async function startTriviaGame(){
  ...
  metrics.forEach(m=>m.dirs.forEach(dir=>{
    ...
    specs=specs.concat(_triviaBuildSpecs(m.sorted,dir,claimed,m.singleQ,m.multiQ,TRIVIA_SINGLE_PER_SIDE,groupN));
  }));
...
async function startRandomChallenge(){
  ...
  _challenge={queue,pos:0,firstTry:0,total:10};
...
function showChallengeResult(){
  ...
  if(g.daily){
    ...
    $('res-l2').textContent=t('timeLbl')+': '+_formatDailyTime(g.timeMs);
    ...
    if(typeof submitDailyChallengeScore==='function'){
      submitDailyChallengeScore(g.date,g.firstTry,g.total,g.timeMs).then(()=>renderDailyLeaderboard(g.date));
    }
...
function updateStats(){
  if(game.challengeMode){
    ...
    else{$('stat-s').textContent='';$('lbl-s').textContent='';}
...
function updateFlagStats(){
  if(game.challengeMode){
    const g=_challenge;
    $('flag-skip-btn').style.display='none';
    ...
    else{$('flag-stat-s').textContent='';$('flag-lbl-s').textContent=t('skippedLbl');}
