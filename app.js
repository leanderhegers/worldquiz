...
function _triviaPanel(){
  return `<div class="gs-grid gs-grid-map">${gsCard('startPopGame()',t('popQuiz'),'👥','pop:world')}${gsCard('startAreaGame()',t('areaQuiz'),'📐','area:world')}</div>`;
}
...
function gameScoreKey(){
  ...
  if(game.areaMode)return 'area:world';
...
function updateAllText(){
  ...
  if(game.current&&C[game.current]&&!(game.riverMode||game.lakeMode||game.cityMode||game.mountainMode||game.rangeMode||game.pinMode||game.popMode||game.areaMode))$('target-name').textContent=cn(game.current);updateStats();
...
// ── AREA QUIZ ──
// Same reuse as the population quiz above, but ranked by land area instead of a fetched stats
// file: the map's own topojson geometry already IS the area data, via d3.geoArea() (steradians,
...
async function startAreaGame(){
  if(!worldData)await loadMap();
  const area=_computeAreaData();
  const ids=Object.keys(C).map(Number).filter(id=>area[id]!=null);
  const sorted=ids.slice().sort((a,b)=>area[b]-area[a]);
  const n=Math.min(AREA_ROUNDS_PER_SIDE,Math.floor(sorted.length/2));
  const rounds=[];
  for(let i=0;i<n;i++)rounds.push({id:sorted[i],q:_areaQuestionText(i+1,'most')});
  for(let i=0;i<n;i++)rounds.push({id:sorted[sorted.length-1-i],q:_areaQuestionText(i+1,'least')});
  const shuffled=shuffle(rounds);
  const areaQuestions={};
  shuffled.forEach(r=>{areaQuestions[r.id]=r.q;});
  game={mode:'area',areaMode:true,areaQuestions,
    queue:shuffled.map(r=>r.id),
    current:null,found:new Set(),correct:0,wrong:0,firstTry:0,
    total:shuffled.length,wrongOnCurrent:false,skipped:0};
  lastMode='area';canClick=true;optsOpen=false;
  showScreen('game-screen');$('opts-panel').style.display='none';
  $('target-name').textContent='';showFeedback(t('load'),'#888');
  updateAllText();
  renderMap(worldData);
