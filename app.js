...
function renderHome(){
  ...
  setTxt('home-play',t('playBtn'));setTxt('home-random-btn',t('randomChallengeBtn'));setTxt('home-options-btn','⚙ '+t('optionsLbl'));
...
function back(){if(_regGame)return void regionBack();if(game.challengeMode){game={};_challenge=null;goHome();return;}const key=game.flagMode?'flag':game.pinMode?'pin':game.cityMode?'city':game.riverMode?'river':game.lakeMode?'lake':(game.mountainMode||game.rangeMode)?'mountain':null;game={};if(key){goToGames(key);return;}_quizBackAction();}
