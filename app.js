...
// Same 6 continent presets, but as [lon, lat, scale] tuned directly under Mercator instead of
// being converted from the Natural-Earth pixel presets above. That conversion (see zoomForMode)
// round-trips through a point's *geographic* coordinates, which is correct in principle, but
// Mercator and Natural-Earth stretch latitude by very different amounts — a pixel anchor picked
// by eye in Natural-Earth space lands at the "wrong" visual spot once re-projected into Mercator
// (worst case: an anchor near the equator in NE space, where NE's own curvature happens to be
// showing higher latitudes, converts to a genuinely low latitude that's nowhere near the intended
// region — e.g. the EU preset landed in the Aegean/Egypt instead of Europe). Hand-verified in the
// live app (screenshot each continent under Mercator, iterate lon/lat/k until it's fully framed).
const ZOOMS_MERCATOR={EU:[12,48,2.05],AF:[18,6,1.7],AS:[85,26,1.15],NA:[-85,32,1.45],SA:[-58,-15,1.55],OC:[168,-20,1.15]};
...
function zoomForMode(mode){
  // Continent presets have their own Mercator-tuned lon/lat/k (see ZOOMS_MERCATOR) — skip the
  // Natural-Earth pixel conversion below entirely rather than feed it through a reprojection that
  // puts the anchor in the wrong place (see comment on ZOOMS_MERCATOR).
  if(projection==='mercator'&&ZOOMS_MERCATOR[mode]){
    const zm=ZOOMS_MERCATOR[mode];
    return{lon:zm[0],lat:zm[1],k:zm[2]};
  }
...
// Colorblind Mode is a toggle in the global options (home screen), not a 4th Design — it's
// orthogonal to which Design is picked, not a competing aesthetic. Applied as an override on
// top of whichever theme is active (Okabe-Ito palette: designed to stay pairwise distinguishable
// under protanopia/deuteranopia/tritanopia, unlike every theme's own found/wrong pair, which
// relies on a red-vs-green distinction those conditions make hard to tell apart). Remembers each
// theme's real values in THEME_ORIGINALS so turning the toggle back off restores them exactly.
const COLORBLIND_OVERRIDE={found:'#009e73',wrong:'#d55e00',skipped:'#cc79a7',hov:'#e69f00',bar:'#009e73'};
const THEME_ORIGINALS={};
for(const k of Object.keys(THEMES))THEME_ORIGINALS[k]={found:THEMES[k].found,wrong:THEMES[k].wrong,skipped:THEMES[k].skipped,hov:THEMES[k].hov,bar:THEMES[k].bar};
let colorblindMode=false;
function _applyColorblindOverride(){
  for(const k of Object.keys(THEMES))Object.assign(THEMES[k],colorblindMode?COLORBLIND_OVERRIDE:THEME_ORIGINALS[k]);
}
function setColorblindMode(v){
  colorblindMode=v;
  _applyColorblindOverride();
  applyTheme();
  if(worldData&&$('game-screen')&&$('game-screen').style.display!=='none'&&game&&Object.keys(game).length)renderMap(worldData);
...
const MODES={world:{de:'Alle Länder',en:'All Countries',es:'Todos los países',fr:'Tous les pays',cnt:_cc('world')},EU:{de:'Europa',en:'Europe',es:'Europa',fr:'Europe',cnt:_cc('EU')},AF:{de:'Afrika',en:'Africa',es:'África',fr:'Afrique',cnt:_cc('AF')},AS:{de:'Asien',en:'Asia',es:'Asia',fr:'Asie',cnt:_cc('AS')},NA:{de:'Nordamerika',en:'N. America',es:'Norteamérica',fr:'Amérique du Nord',cnt:_cc('NA')},SA:{de:'Südamerika',en:'S. America',es:'Sudamérica',fr:'Amérique du Sud',cnt:_cc('SA')},OC:{de:'Ozeanien',en:'Oceania',es:'Oceanía',fr:'Océanie',cnt:_cc('OC')},custom:{de:'Eigener Modus',en:'Custom Mode',es:'Modo Personalizado',fr:'Mode Personnalisé',cnt:'⚙'}};
const TX={
  de:{title:'Weltkarte Quiz',sub:'Ein Ländername erscheint — finde und klicke es auf der Karte.',find:'Finde dieses Land',findLake:'Finde diesen See',findRiver:'Finde diesen Fluss',findCity:'Finde diese Stadt',findMountain:'Finde diesen Gipfel',findRange:'Finde dieses Gebirge',cityQuiz:'Städte-Quiz',citySub:'Ein Stadtname erscheint — finde und klicke sie auf der Karte.',cities:'Städte',cityDiffs:[{key:'easy',label:'Einfach'},{key:'medium',label:'Mittel'},{key:'hard',label:'Schwer'}],cityCountries:[{key:'DE',label:'Deutschland'},{key:'US',label:'USA'},{key:'FR',label:'Frankreich'},{key:'CN',label:'China'},{key:'JP',label:'Japan'},{key:'IN',label:'Indien'},{key:'BR',label:'Brasilien'},{key:'AU',label:'Australien'},{key:'MX',label:'Mexiko'}],pinQuiz:'Drop a Pin',pinSub:'Eine Stadt erscheint — setze die Stecknadel möglichst nah an ihren Standort.',findPin:'Wo liegt diese Stadt?',pinEuLabel:'Europa',pointsLbl:'Punkte',roundsLbl:'Runden',avgLbl:'Ø Distanz',pinFb:(km,pts)=>'📍 '+km+' km · +'+pts+' P',lakeQuiz:'Seen-Quiz',lakeSub:'Ein See erscheint — finde und klicke ihn auf der Karte.',lakeDiffs:[{key:'beginner',label:'Anfänger',count:13},{key:'easy',label:'Einfach',count:44},{key:'medium',label:'Mittel',count:76},{key:'hard',label:'Schwer',count:144}],lakes:'Seen',riverQuiz:'Fluss-Quiz',riverSub:'Ein Flussname erscheint — finde und klicke ihn auf der Karte.',riverDiffs:[{key:'beginner',label:'Anfänger',count:14},{key:'easy',label:'Einfach',count:41},{key:'medium',label:'Mittel',count:80},{key:'hard',label:'Schwer',count:130}],rivers:'Flüsse',mountainQuiz:'Gebirge-Quiz',mountainSub:'Ein Gebirge oder Gipfel erscheint — finde und klicke es auf der Karte.',mountainRanges:'Gebirgszüge',peaks:'Gipfel',rangeDiffs:[{key:'beginner',label:'Sehr leicht',count:10},{key:'easy',label:'Leicht',count:19},{key:'medium',label:'Mittel',count:90},{key:'hard',label:'Schwer',count:200}],mountainDiffs:[{key:'beginner',label:'Sehr leicht',count:10},{key:'easy',label:'Leicht',count:18},{key:'medium',label:'Mittel',count:100},{key:'hard',label:'Schwer',count:250}],flagQuiz:'Flaggen Quiz',flagSub:'Eine Flagge erscheint — tippe den Ländernamen ein.',flagPlaceholder:'Land eingeben …',flagWorld:'Weltweit',flagDiffs:[{key:'beginner',label:'Anfänger'},{key:'easy',label:'Einfach'},{key:'medium',label:'Mittel'},{key:'hard',label:'Schwer'}],homeTitle:'Geografie-Spiele',homeSub:'Teste dein Wissen über die Welt — Länder, Seen, Flüsse und Flaggen.',playBtn:'Spielen',optionsLbl:'Optionen',homeBtn:'← Start',scrollHint:'Scrollen für mehr',mapQuiz:'Weltkarte',load:'Karte wird geladen …',back:'← Menü',foundLbl:'gefunden',correctLbl:'richtig',wrongLbl:'falsch',remLbl:'verbleibend',resTitle:'Quiz abgeschlossen!',again:'Nochmal spielen',newgame:'Zurück zur Auswahl',shareBtn:'📤 Teilen',shareCopied:'In Zwischenablage kopiert!',shareTitle:'Weltkarte Quiz',randomChallengeBtn:'🎲 Random Challenge',dailyChallengeBtn:'📅 Tages-Challenge',dailyLoginRequired:'Anmelden, um die Tages-Challenge zu spielen',timeLbl:'Zeit',dailyDoneTitle:'Heute schon gespielt!',dailyLeaderboardTitle:'Bestenliste heute',dailyHintTitle:'Achtung, Zeit läuft!',dailyHintText:'Die Tages-Challenge läuft auf Zeit — je schneller du fertig bist, desto besser deine Platzierung in der heutigen Bestenliste.',dailyHintDontShow:'Nicht mehr anzeigen',dailyHintBtn:"Los geht's!",langLbl:'Sprache',themeLbl:'Design',colorblindLabel:'Farbenblind-Modus',colorblindTip:'Ersetzt Grün/Rot bei richtig/falsch durch besser unterscheidbare Farben, unabhängig vom gewählten Design.',colorblindOn:'An',colorblindOff:'Aus',projLbl:'Kartenansicht',projStd:'Natural Earth',projMerc:'Mercator',foundModeLabel:'Richtig geraten',keepOn:'Grün markiert',keepOff:'Ausgeblendet',wrongHintLabel:'Hinweise bei Fehlern',wrongHintTip:'Nach einer falschen Antwort wird das richtige Ziel kurz angezeigt.',wrongHintOn:'Anzeigen',wrongHintOff:'Ausblenden',skipHintLabel:'Hinweise beim Überspringen',skipHintTip:'Zeigt nach dem Überspringen das übersprungene Ziel kurz an.',skipHintOn:'Anzeigen',skipHintOff:'Ausblenden',countries:'Länder',zoomTip:'Zoom & Pan möglich',skipLbl:'Überspringen',skippedLbl:'übersprungen',correctFb:n=>'✓ Richtig! '+n,wrongFb:n=>'✗ Das war '+n,res1:(a,b,c)=>`${a} von ${b} beim 1. Versuch (${c} %)`,res2:(a,b)=>`${a} richtig, ${b} daneben`,challengeScore:n=>'Punktzahl: '+n,themes:{atlas:'Atlas',neon:'Cyberpunk',terrain:'Terrain'},cback:'← Zurück',ctitle:'Eigener Modus',clblCont:'Kontinente',clblCount:'Anzahl Länder',clblOf:'verfügbar',clblAll:'Alle Länder',clblCountries:'Länder wählen',cbtnAll:'Alle',cbtnNone:'Keine',cbtnStart:'Starten',signIn:'Anmelden',signUp:'Registrieren',signOut:'Abmelden',loginTitle:'Anmelden',registerTitle:'Konto erstellen',authEmail:'E-Mail',authPw:'Passwort',authName:'Anzeigename (optional)',loggedIn:'Angemeldet als',toRegister:'Noch kein Konto? Registrieren',toLogin:'Bereits ein Konto? Anmelden',authFillAll:'Bitte E-Mail und Passwort eingeben.',authNotConfigured:'Firebase ist noch nicht konfiguriert.',authErrEmail:'Ungültige E-Mail-Adresse.',authErrInUse:'Diese E-Mail wird bereits verwendet.',authErrWeak:'Passwort zu schwach (min. 6 Zeichen).',authErrCred:'E-Mail oder Passwort falsch.',authErrGeneric:'Etwas ist schiefgelaufen. Bitte erneut versuchen.',authUsername:'Benutzername',showPw:'Passwort anzeigen',forgotPw:'Passwort vergessen?',resetSent:'E-Mail zum Zurücksetzen wurde gesendet.',enterEmailFirst:'Bitte zuerst deine E-Mail eingeben.',authUserRequired:'Bitte einen Benutzernamen wählen.',authUserInvalid:'3–20 Zeichen: Buchstaben, Zahlen, _',authUserTaken:'Benutzername bereits vergeben.',googleBtn:'Mit Google anmelden',orSep:'oder',newRecord:'Neuer Rekord!',bestLabel:'Bestwert',roundLimitLbl:'Anzahl Ziele',allTargetsLbl:'Alle Ziele',normalMode:'Normaler Modus',learnModeLbl:'Lernmodus',learnModeLogin:'Anmelden für Lernmodus',learnModeDesc:'Häufig falsch gemachte Fragen kommen zuerst. Themen, die du bereits 3× gemeistert hast, werden ausgeblendet.',learnModeCta:'→ Klicken zum Anmelden',inputQuiz:'Errate das Land',inputSub:'Flagge, Hauptstadt oder Umriss — tippe den richtigen Namen ein.',flagCardSub:'Welche Flagge ist das?',capitalQuiz:'Hauptstadt-Quiz',capitalCardSub:'Land ↔ Hauptstadt',outlineQuiz:'Umriss-Quiz',outlineCardSub:'Erkenne die Landesform',languageQuiz:'Sprachen-Quiz',languageCardSub:'Welche Sprache ist das?',currencyQuiz:'Währungs-Quiz',currencyCardSub:'Welche Währung ist das?',wappenQuiz:'Wappen-Quiz',wappenCardSub:'Welches Wappen ist das?',neighborQuiz:'Nachbarland-Rätsel',neighborCardSub:'Welches Land grenzt an diese?',iqAskLanguage:'Welche Sprache ist das?',iqAskCurrency:'Welche Währung ist das?',iqAskNeighbor:'Welches Land grenzt an diese Länder?',langPlaceholder:'Sprache eingeben …',curPlaceholder:'Währung eingeben …',languages:'Sprachen',currencies:'Währungen',cfgRegion:'Region',cfgDiff:'Schwierigkeit',cfgDir:'Richtung',cfgStart:'Quiz starten',dirC2Cap:'Land → Hauptstadt',dirCap2C:'Hauptstadt → Land',dirFlag2Cap:'Flagge → Hauptstadt',diffAll:'Alle',regWorld:'Welt',iqAskCapital:'Wie heißt die Hauptstadt?',iqAskCountry:'Welches Land?',capPlaceholder:'Hauptstadt eingeben …',cfgDropdownLabel:'Vorschläge',cfgDropdownOn:'Anzeigen',cfgDropdownOff:'Ausblenden',cfgSkipHintLabel:'Hinweis beim Überspringen',cfgSkipHintOn:'Anzeigen',cfgSkipHintOff:'Ausblenden',regionQuiz:'Regionen',regionSub:'Ein Regionsname erscheint — finde und klicke sie auf der Karte.',findRegion:'Finde diese Region',regions:'Regionen',popQuiz:'Bevölkerung',popSub:'Die meist- oder wenigst-bevölkerten Länder — finde und klicke sie auf der Karte.',findPop:'Welches Land?',popMostQ:'Das bevölkerungsreichste Land der Welt',popLeastQ:'Das bevölkerungsärmste Land der Welt',popMostRankQ:n=>'Platz '+n+' der bevölkerungsreichsten Länder der Welt',popLeastRankQ:n=>'Platz '+n+' der bevölkerungsärmsten Länder der Welt',areaQuiz:'Fläche',findArea:'Welches Land?',findTrivia:'Welches Land?',areaMostQ:'Das flächenmäßig größte Land der Welt',areaLeastQ:'Das flächenmäßig kleinste Land der Welt',areaMostRankQ:n=>'Platz '+n+' der flächenmäßig größten Länder der Welt',areaLeastRankQ:n=>'Platz '+n+' der flächenmäßig kleinsten Länder der Welt',popMostNQ:n=>'Die '+n+' bevölkerungsreichsten Länder der Welt',popLeastNQ:n=>'Die '+n+' bevölkerungsärmsten Länder der Welt',areaMostNQ:n=>'Die '+n+' flächenmäßig größten Länder der Welt',areaLeastNQ:n=>'Die '+n+' flächenmäßig kleinsten Länder der Welt',happyMostQ:'Das glücklichste Land der Welt',happyLeastQ:'Das unglücklichste Land der Welt',happyMostRankQ:n=>'Platz '+n+' der glücklichsten Länder der Welt',happyLeastRankQ:n=>'Platz '+n+' der unglücklichsten Länder der Welt',happyMostNQ:n=>'Die '+n+' glücklichsten Länder der Welt',happyLeastNQ:n=>'Die '+n+' unglücklichsten Länder der Welt',exportMostQ:'Das Land mit dem höchsten Exportvolumen der Welt',exportLeastQ:'Das Land mit dem niedrigsten Exportvolumen der Welt',exportMostRankQ:n=>'Platz '+n+' beim weltweiten Exportvolumen',exportLeastRankQ:n=>'Platz '+n+' von hinten beim weltweiten Exportvolumen',exportMostNQ:n=>'Die '+n+' Länder mit dem höchsten Exportvolumen der Welt',exportLeastNQ:n=>'Die '+n+' Länder mit dem niedrigsten Exportvolumen der Welt',heritageMostQ:'Das Land mit den meisten UNESCO-Welterbestätten',heritageMostRankQ:n=>'Platz '+n+' bei den meisten UNESCO-Welterbestätten',heritageMostNQ:n=>'Die '+n+' Länder mit den meisten UNESCO-Welterbestätten',findTriviaN:'Welche Länder?',catHeading:'Was möchtest du spielen?',catSub:'Wähle eine Spielart aus',catClickTitle:'Auf der Karte anklicken',catClickSub:'Länder, Regionen und Trivia auf der Weltkarte finden',catPinTitle:'Drop-a-Pin',catPinSub:'Stecknadel möglichst nah an den Standort setzen',catInputTitle:'Texteingabe & Multiple Choice',catInputSub:'Flaggen, Wappen, Hauptstädte und Umrisse',catOtherTitle:'Weitere Weltkarten-Quizzes',catOtherSub:'Städte, Flüsse, Seen und Gebirge anklicken',clickRootHeading:'Auf der Karte anklicken',clickRootSub:'Länder oder Regionen?',landerTitle:'Länder',landerCardSub:'Weltkarte, Kontinente, Trivia & mehr',regionCardSub:'Bundesländer, Staaten & mehr',landerRootSub:'Wähle eine Spielart',optWorld:'Weltkarte',optWorldSub:'Alle 197 Länder',optContinents:'Kontinente',continentsSub:'Wähle einen Kontinent',optDifficulty:'Schwierigkeit',difficultySub:'Länder nach Bevölkerung sortiert',diffTierLabels:['Sehr leicht','Leicht','Mittel','Schwer','Sehr schwer'],comingSoon:'Bald verfügbar',optTrivia:'Trivia',triviaSub:'Besondere Fragen zu Ländern',optCustom:'Eigener Modus',optCustomSub:'Kontinente & Anzahl frei wählen',pinTriviaLbl:'Trivia',landmarks:'Orte',findPinTrivia:'Wo liegt das?',perQuizLbl:'pro Quiz',wrongShort:'Falsch!'},
  en:{title:'World Map Quiz',sub:'A country name appears — find and click it on the map.',find:'Find this country',findLake:'Find this lake',findRiver:'Find this river',findCity:'Find this city',findMountain:'Find this peak',findRange:'Find this mountain range',cityQuiz:'City Quiz',citySub:'A city name appears — find and click it on the map.',cities:'Cities',cityDiffs:[{key:'easy',label:'Easy'},{key:'medium',label:'Medium'},{key:'hard',label:'Hard'}],cityCountries:[{key:'DE',label:'Germany'},{key:'US',label:'USA'},{key:'FR',label:'France'},{key:'CN',label:'China'},{key:'JP',label:'Japan'},{key:'IN',label:'India'},{key:'BR',label:'Brazil'},{key:'AU',label:'Australia'},{key:'MX',label:'Mexico'}],pinQuiz:'Drop a Pin',pinSub:'A city appears — drop the pin as close to its location as you can.',findPin:'Where is this city?',pinEuLabel:'Europe',pointsLbl:'Points',roundsLbl:'Rounds',avgLbl:'Avg distance',pinFb:(km,pts)=>'📍 '+km+' km · +'+pts+' P',lakeQuiz:'Lake Quiz',lakeSub:'A lake name appears — find and click it on the map.',lakeDiffs:[{key:'beginner',label:'Beginner',count:13},{key:'easy',label:'Easy',count:44},{key:'medium',label:'Medium',count:76},{key:'hard',label:'Hard',count:144}],lakes:'Lakes',riverQuiz:'River Quiz',riverSub:'A river name appears — find and click it on the map.',riverDiffs:[{key:'beginner',label:'Beginner',count:14},{key:'easy',label:'Easy',count:41},{key:'medium',label:'Medium',count:80},{key:'hard',label:'Hard',count:130}],rivers:'Rivers',mountainQuiz:'Mountain Quiz',mountainSub:'A mountain range or peak appears — find and click it on the map.',mountainRanges:'Mountain Ranges',peaks:'Peaks',rangeDiffs:[{key:'beginner',label:'Very Easy',count:10},{key:'easy',label:'Easy',count:19},{key:'medium',label:'Medium',count:90},{key:'hard',label:'Hard',count:200}],mountainDiffs:[{key:'beginner',label:'Very Easy',count:10},{key:'easy',label:'Easy',count:18},{key:'medium',label:'Medium',count:100},{key:'hard',label:'Hard',count:250}],flagQuiz:'Flag Quiz',flagSub:'A flag appears — type the country name.',flagPlaceholder:'Enter country …',flagWorld:'Worldwide',flagDiffs:[{key:'beginner',label:'Beginner'},{key:'easy',label:'Easy'},{key:'medium',label:'Medium'},{key:'hard',label:'Hard'}],homeTitle:'World Geography Games',homeSub:'Test your knowledge of the world — countries, lakes, rivers and flags.',playBtn:'Play',optionsLbl:'Options',homeBtn:'← Home',scrollHint:'Scroll for more',mapQuiz:'World Map',load:'Loading map …',back:'← Menu',foundLbl:'found',correctLbl:'correct',wrongLbl:'wrong',remLbl:'remaining',resTitle:'Quiz complete!',again:'Play again',newgame:'Back to selection',shareBtn:'📤 Share',shareCopied:'Copied to clipboard!',shareTitle:'World Map Quiz',randomChallengeBtn:'🎲 Random Challenge',dailyChallengeBtn:'📅 Daily Challenge',dailyLoginRequired:'Sign in to play the Daily Challenge',timeLbl:'Time',dailyDoneTitle:'Already played today!',dailyLeaderboardTitle:"Today's leaderboard",dailyHintTitle:"Heads up, it's timed!",dailyHintText:"The Daily Challenge is timed — the faster you finish, the better your spot on today's leaderboard.",dailyHintDontShow:"Don't show again",dailyHintBtn:"Let's go!",langLbl:'Language',themeLbl:'Theme',colorblindLabel:'Colorblind Mode',colorblindTip:'Replaces green/red for correct/wrong with more distinguishable colors, independent of the chosen theme.',colorblindOn:'On',colorblindOff:'Off',projLbl:'Map view',projStd:'Natural Earth',projMerc:'Mercator',foundModeLabel:'Correct answers',keepOn:'Stay green',keepOff:'Fade out',wrongHintLabel:'Hints on errors',wrongHintTip:'After a wrong answer, the correct country is briefly revealed.',wrongHintOn:'Show',wrongHintOff:'Hide',skipHintLabel:'Skip hints',skipHintTip:'Shows the skipped target briefly after skipping.',skipHintOn:'Show',skipHintOff:'Hide',countries:'countries',zoomTip:'Zoom & pan supported',skipLbl:'Skip',skippedLbl:'skipped',correctFb:n=>'✓ Correct! '+n,wrongFb:n=>'That was '+n,res1:(a,b,c)=>`${a} of ${b} on first try (${c}%)`,res2:(a,b)=>`${a} correct, ${b} missed`,challengeScore:n=>'Score: '+n,themes:{atlas:'Atlas',neon:'Cyberpunk',terrain:'Terrain'},cback:'← Back',ctitle:'Custom Mode',clblCont:'Continents',clblCount:'Number of countries',clblOf:'available',clblAll:'All countries',clblCountries:'Select countries',cbtnAll:'All',cbtnNone:'None',cbtnStart:'Start',signIn:'Sign in',signUp:'Sign up',signOut:'Sign out',loginTitle:'Sign in',registerTitle:'Create account',authEmail:'Email',authPw:'Password',authName:'Display name (optional)',loggedIn:'Signed in as',toRegister:"No account? Sign up",toLogin:'Already have an account? Sign in',authFillAll:'Please enter email and password.',authNotConfigured:'Firebase is not configured yet.',authErrEmail:'Invalid email address.',authErrInUse:'This email is already in use.',authErrWeak:'Password too weak (min. 6 characters).',authErrCred:'Wrong email or password.',authErrGeneric:'Something went wrong. Please try again.',authUsername:'Username',showPw:'Show password',forgotPw:'Forgot password?',resetSent:'Password reset email sent.',enterEmailFirst:'Please enter your email first.',authUserRequired:'Please choose a username.',authUserInvalid:'3–20 chars: letters, numbers, _',authUserTaken:'Username already taken.',googleBtn:'Sign in with Google',orSep:'or',newRecord:'New record!',bestLabel:'Best',roundLimitLbl:'Round limit',allTargetsLbl:'All targets',normalMode:'Normal Mode',learnModeLbl:'Learn Mode',learnModeLogin:'Sign in for Learn Mode',learnModeDesc:'Frequent mistakes appear first. Topics you\'ve mastered 3× are hidden.',learnModeCta:'→ Click to sign in',inputQuiz:'Name the country',inputSub:'Flag, capital or outline — type the right name.',flagCardSub:'Which flag is this?',capitalQuiz:'Capital Quiz',capitalCardSub:'Country ↔ capital',outlineQuiz:'Outline Quiz',outlineCardSub:'Recognise the shape',languageQuiz:'Language Quiz',languageCardSub:'Which language is this?',currencyQuiz:'Currency Quiz',currencyCardSub:'Which currency is this?',wappenQuiz:'Coat of Arms Quiz',wappenCardSub:'Which coat of arms is this?',neighborQuiz:'Neighbor Puzzle',neighborCardSub:'Which country do these border?',iqAskLanguage:'Which language is this?',iqAskCurrency:'Which currency is this?',iqAskNeighbor:'Which country do these border?',langPlaceholder:'Enter language …',curPlaceholder:'Enter currency …',languages:'languages',currencies:'currencies',cfgRegion:'Region',cfgDiff:'Difficulty',cfgDir:'Direction',cfgStart:'Start quiz',dirC2Cap:'Country → capital',dirCap2C:'Capital → country',dirFlag2Cap:'Flag → capital',diffAll:'All',regWorld:'World',iqAskCapital:'What is the capital?',iqAskCountry:'Which country?',capPlaceholder:'Enter capital …',cfgDropdownLabel:'Suggestions',cfgDropdownOn:'Show',cfgDropdownOff:'Hide',cfgSkipHintLabel:'Hint on skip',cfgSkipHintOn:'Show',cfgSkipHintOff:'Hide',regionQuiz:'Regions',regionSub:'A region name appears — find and click it on the map.',findRegion:'Find this region',regions:'Regions',popQuiz:'Population',popSub:'The most or least populous countries — find and click them on the map.',findPop:'Which country?',popMostQ:'The most populous country in the world',popLeastQ:'The least populous country in the world',popMostRankQ:n=>'#'+n+' most populous country in the world',popLeastRankQ:n=>'#'+n+' least populous country in the world',areaQuiz:'Area',findArea:'Which country?',findTrivia:'Which country?',areaMostQ:'The largest country in the world by area',areaLeastQ:'The smallest country in the world by area',areaMostRankQ:n=>'#'+n+' largest country in the world by area',areaLeastRankQ:n=>'#'+n+' smallest country in the world by area',popMostNQ:n=>'The '+n+' most populous countries in the world',popLeastNQ:n=>'The '+n+' least populous countries in the world',areaMostNQ:n=>'The '+n+' largest countries in the world by area',areaLeastNQ:n=>'The '+n+' smallest countries in the world by area',happyMostQ:'The happiest country in the world',happyLeastQ:'The unhappiest country in the world',happyMostRankQ:n=>'#'+n+' happiest country in the world',happyLeastRankQ:n=>'#'+n+' unhappiest country in the world',happyMostNQ:n=>'The '+n+' happiest countries in the world',happyLeastNQ:n=>'The '+n+' unhappiest countries in the world',exportMostQ:'The country with the highest export volume in the world',exportLeastQ:'The country with the lowest export volume in the world',exportMostRankQ:n=>'#'+n+' in the world by export volume',exportLeastRankQ:n=>'#'+n+' from the bottom in the world by export volume',exportMostNQ:n=>'The '+n+' countries with the highest export volume in the world',exportLeastNQ:n=>'The '+n+' countries with the lowest export volume in the world',heritageMostQ:'The country with the most UNESCO World Heritage Sites',heritageMostRankQ:n=>'#'+n+' for the most UNESCO World Heritage Sites',heritageMostNQ:n=>'The '+n+' countries with the most UNESCO World Heritage Sites',findTriviaN:'Which countries?',catHeading:'What do you want to play?',catSub:'Choose a game type',catClickTitle:'Click on the map',catClickSub:'Find countries, regions and trivia on the world map',catPinTitle:'Drop a Pin',catPinSub:'Drop the pin as close to the location as you can',catInputTitle:'Text Input & Multiple Choice',catInputSub:'Flags, coats of arms, capitals and outlines',catOtherTitle:'More World Map Quizzes',catOtherSub:'Click cities, rivers, lakes and mountains',clickRootHeading:'Click on the map',clickRootSub:'Countries or regions?',landerTitle:'Countries',landerCardSub:'World map, continents, trivia & more',regionCardSub:'States, provinces & more',landerRootSub:'Choose a game type',optWorld:'World Map',optWorldSub:'All 197 countries',optContinents:'Continents',continentsSub:'Choose a continent',optDifficulty:'Difficulty',difficultySub:'Countries sorted by population',diffTierLabels:['Very Easy','Easy','Medium','Hard','Very Hard'],comingSoon:'Coming soon',optTrivia:'Trivia',triviaSub:'Special questions about countries',optCustom:'Custom Mode',optCustomSub:'Pick continents & country count',pinTriviaLbl:'Trivia',landmarks:'Places',findPinTrivia:'Where is this?',perQuizLbl:'per quiz',wrongShort:'Wrong!'},
  es:{title:'Quiz del Mapa Mundial',sub:'Aparece el nombre de un país — encuéntralo y haz clic en el mapa.', find:'Encuentra este país',findLake:'Encuentra este lago',findRiver:'Encuentra este río',findCity:'Encuentra esta ciudad',findMountain:'Encuentra este pico',findRange:'Encuentra esta cordillera', cityQuiz:'Quiz de Ciudades',citySub:'Aparece el nombre de una ciudad — encuéntrala y haz clic en el mapa.',cities:'Ciudades', cityDiffs:[{key:'easy',label:'Fácil'},{key:'medium',label:'Medio'},{key:'hard',label:'Difícil'}], cityCountries:[{key:'DE',label:'Alemania'},{key:'US',label:'EE. UU.'},{key:'FR',label:'Francia'},{key:'CN',label:'China'},{key:'JP',label:'Japón'},{key:'IN',label:'India'},{key:'BR',label:'Brasil'},{key:'AU',label:'Australia'},{key:'MX',label:'México'}], pinQuiz:'Clava el Pin',pinSub:'Aparece una ciudad — coloca el marcador lo más cerca posible de su ubicación.',findPin:'¿Dónde está esta ciudad?',pinEuLabel:'Europa',pointsLbl:'Puntos',roundsLbl:'Rondas',avgLbl:'Dist. media',pinFb:(km,pts)=>'📍 '+km+' km · +'+pts+' P', lakeQuiz:'Quiz de Lagos',lakeSub:'Aparece el nombre de un lago — encuéntralo y haz clic en el mapa.', lakeDiffs:[{key:'beginner',label:'Principiante',count:13},{key:'easy',label:'Fácil',count:44},{key:'medium',label:'Medio',count:76},{key:'hard',label:'Difícil',count:144}],lakes:'Lagos', riverQuiz:'Quiz de Ríos',riverSub:'Aparece el nombre de un río — encuéntralo y haz clic en el mapa.', riverDiffs:[{key:'beginner',label:'Principiante',count:14},{key:'easy',label:'Fácil',count:41},{key:'medium',label:'Medio',count:80},{key:'hard',label:'Difícil',count:130}],rivers:'Ríos', mountainQuiz:'Quiz de Montañas',mountainSub:'Aparece una cordillera o un pico — encuéntralo y haz clic en el mapa.',mountainRanges:'Cordilleras',peaks:'Picos', rangeDiffs:[{key:'beginner',label:'Muy fácil',count:10},{key:'easy',label:'Fácil',count:19},{key:'medium',label:'Medio',count:90},{key:'hard',label:'Difícil',count:200}], mountainDiffs:[{key:'beginner',label:'Muy fácil',count:10},{key:'easy',label:'Fácil',count:18},{key:'medium',label:'Medio',count:100},{key:'hard',label:'Difícil',count:250}], flagQuiz:'Quiz de Banderas',flagSub:'Aparece una bandera — escribe el nombre del país.',flagPlaceholder:'Escribe el país …',flagWorld:'Mundial', flagDiffs:[{key:'beginner',label:'Principiante'},{key:'easy',label:'Fácil'},{key:'medium',label:'Medio'},{key:'hard',label:'Difícil'}], homeTitle:'Juegos de Geografía',homeSub:'Pon a prueba tu conocimiento del mundo — países, lagos, ríos y banderas.', playBtn:'Jugar',optionsLbl:'Opciones',homeBtn:'← Inicio',scrollHint:'Desplázate para ver más',mapQuiz:'Mapa Mundial',load:'Cargando mapa …',back:'← Menú', foundLbl:'encontrados',correctLbl:'correctas',wrongLbl:'incorrectas',remLbl:'restantes',resTitle:'¡Quiz completado!',again:'Jugar de nuevo',newgame:'Volver a la selección', shareBtn:'📤 Compartir',shareCopied:'¡Copiado al portapapeles!',shareTitle:'Quiz del Mapa Mundial', randomChallengeBtn:'🎲 Reto Aleatorio',dailyChallengeBtn:'📅 Reto Diario',dailyLoginRequired:'Inicia sesión para jugar el Reto Diario',timeLbl:'Tiempo',dailyDoneTitle:'¡Ya jugaste hoy!',dailyLeaderboardTitle:'Clasificación de hoy',dailyHintTitle:'¡Atención, va contra el reloj!',dailyHintText:'El Reto Diario va contra el reloj — cuanto más rápido termines, mejor tu puesto en la clasificación de hoy.',dailyHintDontShow:'No volver a mostrar',dailyHintBtn:'¡Vamos!', langLbl:'Idioma',themeLbl:'Diseño',colorblindLabel:'Modo daltónico',colorblindTip:'Sustituye el verde/rojo de correcto/incorrecto por colores más distinguibles, sin importar el diseño elegido.',colorblindOn:'Activado',colorblindOff:'Desactivado', projLbl:'Vista del mapa',projStd:'Natural Earth',projMerc:'Mercator', foundModeLabel:'Respuestas correctas',keepOn:'Se mantiene en verde',keepOff:'Se desvanece', wrongHintLabel:'Pistas en errores',wrongHintTip:'Tras una respuesta incorrecta, se muestra brevemente el país correcto.',wrongHintOn:'Mostrar',wrongHintOff:'Ocultar', skipHintLabel:'Pistas al saltar',skipHintTip:'Muestra brevemente el objetivo saltado después de saltarlo.',skipHintOn:'Mostrar',skipHintOff:'Ocultar', countries:'países',zoomTip:'Zoom y desplazamiento disponibles',skipLbl:'Saltar',skippedLbl:'saltadas', correctFb:n=>'✓ ¡Correcto! '+n,wrongFb:n=>'Era '+n, res1:(a,b,c)=>`${a} de ${b} al primer intento (${c} %)`,res2:(a,b)=>`${a} correctas, ${b} falladas`,challengeScore:n=>'Puntuación: '+n, themes:{atlas:'Atlas',neon:'Cyberpunk',terrain:'Terreno'}, cback:'← Volver',ctitle:'Modo Personalizado',clblCont:'Continentes',clblCount:'Número de países',clblOf:'disponibles',clblAll:'Todos los países',clblCountries:'Elegir países', cbtnAll:'Todos',cbtnNone:'Ninguno',cbtnStart:'Empezar', signIn:'Iniciar sesión',signUp:'Registrarse',signOut:'Cerrar sesión',loginTitle:'Iniciar sesión',registerTitle:'Crear cuenta', authEmail:'Correo electrónico',authPw:'Contraseña',authName:'Nombre visible (opcional)',loggedIn:'Sesión iniciada como', toRegister:'¿Sin cuenta? Regístrate',toLogin:'¿Ya tienes cuenta? Inicia sesión', authFillAll:'Introduce correo y contraseña.',authNotConfigured:'Firebase aún no está configurado.', authErrEmail:'Dirección de correo no válida.',authErrInUse:'Este correo ya está en uso.',authErrWeak:'Contraseña demasiado débil (mín. 6 caracteres).', authErrCred:'Correo o contraseña incorrectos.',authErrGeneric:'Algo salió mal. Inténtalo de nuevo.', authUsername:'Nombre de usuario',showPw:'Mostrar contraseña',forgotPw:'¿Olvidaste tu contraseña?',resetSent:'Se ha enviado un correo para restablecerla.', enterEmailFirst:'Introduce primero tu correo electrónico.',authUserRequired:'Elige un nombre de usuario.',authUserInvalid:'3–20 caracteres: letras, números, _',authUserTaken:'Nombre de usuario ya en uso.', googleBtn:'Iniciar sesión con Google',orSep:'o',newRecord:'¡Nuevo récord!',bestLabel:'Mejor marca', roundLimitLbl:'Número de objetivos',allTargetsLbl:'Todos los objetivos',normalMode:'Modo normal', learnModeLbl:'Modo aprendizaje',learnModeLogin:'Inicia sesión para el Modo aprendizaje',learnModeDesc:'Los errores frecuentes aparecen primero. Los temas que ya dominas 3 veces se ocultan.',learnModeCta:'→ Haz clic para iniciar sesión', inputQuiz:'Adivina el país',inputSub:'Bandera, capital o silueta — escribe el nombre correcto.',flagCardSub:'¿Qué bandera es esta?', capitalQuiz:'Quiz de Capitales',capitalCardSub:'País ↔ capital',outlineQuiz:'Quiz de Siluetas',outlineCardSub:'Reconoce la forma', languageQuiz:'Quiz de Idiomas',languageCardSub:'¿Qué idioma es este?',currencyQuiz:'Quiz de Monedas',currencyCardSub:'¿Qué moneda es esta?', wappenQuiz:'Quiz de Escudos',wappenCardSub:'¿Qué escudo es este?',neighborQuiz:'Rompecabezas de Vecinos',neighborCardSub:'¿Qué país limita con estos?', iqAskLanguage:'¿Qué idioma es este?',iqAskCurrency:'¿Qué moneda es esta?',iqAskNeighbor:'¿Qué país limita con estos países?', langPlaceholder:'Escribe el idioma …',curPlaceholder:'Escribe la moneda …',languages:'idiomas',currencies:'monedas', cfgRegion:'Región',cfgDiff:'Dificultad',cfgDir:'Dirección',cfgStart:'Iniciar quiz', dirC2Cap:'País → capital',dirCap2C:'Capital → país',dirFlag2Cap:'Bandera → capital',diffAll:'Todos',regWorld:'Mundo', iqAskCapital:'¿Cómo se llama la capital?',iqAskCountry:'¿Qué país?',capPlaceholder:'Escribe la capital …', cfgDropdownLabel:'Sugerencias',cfgDropdownOn:'Mostrar',cfgDropdownOff:'Ocultar', cfgSkipHintLabel:'Pista al saltar',cfgSkipHintOn:'Mostrar',cfgSkipHintOff:'Ocultar', regionQuiz:'Regiones',regionSub:'Aparece el nombre de una región — encuéntrala y haz clic en el mapa.',findRegion:'Encuentra esta región',regions:'Regiones', popQuiz:'Población',popSub:'Los países más o menos poblados — encuéntralos y haz clic en el mapa.',findPop:'¿Qué país?', popMostQ:'El país más poblado del mundo',popLeastQ:'El país menos poblado del mundo', popMostRankQ:n=>'Puesto '+n+' entre los países más poblados del mundo',popLeastRankQ:n=>'Puesto '+n+' entre los países menos poblados del mundo', areaQuiz:'Superficie',findArea:'¿Qué país?',findTrivia:'¿Qué país?',areaMostQ:'El país más grande del mundo por superficie',areaLeastQ:'El país más pequeño del mundo por superficie', areaMostRankQ:n=>'Puesto '+n+' entre los países más grandes del mundo por superficie',areaLeastRankQ:n=>'Puesto '+n+' entre los países más pequeños del mundo por superficie',popMostNQ:n=>'Los '+n+' países más poblados del mundo',popLeastNQ:n=>'Los '+n+' países menos poblados del mundo',areaMostNQ:n=>'Los '+n+' países más grandes del mundo por superficie',areaLeastNQ:n=>'Los '+n+' países más pequeños del mundo por superficie',happyMostQ:'El país más feliz del mundo',happyLeastQ:'El país menos feliz del mundo',happyMostRankQ:n=>'Puesto '+n+' de los países más felices del mundo',happyLeastRankQ:n=>'Puesto '+n+' de los países menos felices del mundo',happyMostNQ:n=>'Los '+n+' países más felices del mundo',happyLeastNQ:n=>'Los '+n+' países menos felices del mundo',exportMostQ:'El país con el mayor volumen de exportaciones del mundo',exportLeastQ:'El país con el menor volumen de exportaciones del mundo',exportMostRankQ:n=>'Puesto '+n+' del mundo por volumen de exportaciones',exportLeastRankQ:n=>'Puesto '+n+' empezando por el final por volumen de exportaciones',exportMostNQ:n=>'Los '+n+' países con mayor volumen de exportaciones del mundo',exportLeastNQ:n=>'Los '+n+' países con menor volumen de exportaciones del mundo',heritageMostQ:'El país con más sitios Patrimonio de la Humanidad de la UNESCO',heritageMostRankQ:n=>'Puesto '+n+' en sitios Patrimonio de la Humanidad de la UNESCO',heritageMostNQ:n=>'Los '+n+' países con más sitios Patrimonio de la Humanidad de la UNESCO',findTriviaN:'¿Qué países?', catHeading:'¿A qué quieres jugar?',catSub:'Elige un tipo de juego', catClickTitle:'Hacer clic en el mapa',catClickSub:'Encuentra países, regiones y curiosidades en el mapa mundial', catPinTitle:'Clava el Pin',catPinSub:'Coloca el marcador lo más cerca posible de la ubicación', catInputTitle:'Texto y opción múltiple',catInputSub:'Banderas, escudos, capitales y siluetas', catOtherTitle:'Más quizzes del mapa mundial',catOtherSub:'Haz clic en ciudades, ríos, lagos y montañas', clickRootHeading:'Hacer clic en el mapa',clickRootSub:'¿Países o regiones?', landerTitle:'Países',landerCardSub:'Mapa mundial, continentes, curiosidades y más',regionCardSub:'Estados, provincias y más',landerRootSub:'Elige un tipo de juego', optWorld:'Mapa Mundial',optWorldSub:'Los 197 países',optContinents:'Continentes',continentsSub:'Elige un continente', optDifficulty:'Dificultad',difficultySub:'Países ordenados por población',diffTierLabels:['Muy fácil','Fácil','Medio','Difícil','Muy difícil'], comingSoon:'Próximamente',optTrivia:'Curiosidades',triviaSub:'Preguntas especiales sobre países', optCustom:'Modo Personalizado',optCustomSub:'Elige continentes y número de países', pinTriviaLbl:'Curiosidades',landmarks:'Lugares',findPinTrivia:'¿Dónde está esto?', perQuizLbl:'por quiz',wrongShort:'¡Incorrecto!'},
  fr:{title:'Quiz de la Carte du Monde',sub:'Le nom d’un pays apparaît — trouve-le et clique dessus sur la carte.', find:'Trouve ce pays',findLake:'Trouve ce lac',findRiver:'Trouve ce fleuve',findCity:'Trouve cette ville',findMountain:'Trouve ce sommet',findRange:'Trouve cette chaîne de montagnes', cityQuiz:'Quiz des Villes',citySub:'Le nom d’une ville apparaît — trouve-la et clique dessus sur la carte.',cities:'Villes', cityDiffs:[{key:'easy',label:'Facile'},{key:'medium',label:'Moyen'},{key:'hard',label:'Difficile'}], cityCountries:[{key:'DE',label:'Allemagne'},{key:'US',label:'États-Unis'},{key:'FR',label:'France'},{key:'CN',label:'Chine'},{key:'JP',label:'Japon'},{key:'IN',label:'Inde'},{key:'BR',label:'Brésil'},{key:'AU',label:'Australie'},{key:'MX',label:'Mexique'}], pinQuiz:'Plante le Pin',pinSub:'Une ville apparaît — place le repère le plus près possible de son emplacement.',findPin:'Où se trouve cette ville ?',pinEuLabel:'Europe',pointsLbl:'Points',roundsLbl:'Manches',avgLbl:'Distance moy.',pinFb:(km,pts)=>'📍 '+km+' km · +'+pts+' P', lakeQuiz:'Quiz des Lacs',lakeSub:'Le nom d’un lac apparaît — trouve-le et clique dessus sur la carte.', lakeDiffs:[{key:'beginner',label:'Débutant',count:13},{key:'easy',label:'Facile',count:44},{key:'medium',label:'Moyen',count:76},{key:'hard',label:'Difficile',count:144}],lakes:'Lacs', riverQuiz:'Quiz des Fleuves',riverSub:'Le nom d’un fleuve apparaît — trouve-le et clique dessus sur la carte.', riverDiffs:[{key:'beginner',label:'Débutant',count:14},{key:'easy',label:'Facile',count:41},{key:'medium',label:'Moyen',count:80},{key:'hard',label:'Difficile',count:130}],rivers:'Fleuves', mountainQuiz:'Quiz des Montagnes',mountainSub:'Une chaîne de montagnes ou un sommet apparaît — trouve-le et clique dessus sur la carte.',mountainRanges:'Chaînes de montagnes',peaks:'Sommets', rangeDiffs:[{key:'beginner',label:'Très facile',count:10},{key:'easy',label:'Facile',count:19},{key:'medium',label:'Moyen',count:90},{key:'hard',label:'Difficile',count:200}], mountainDiffs:[{key:'beginner',label:'Très facile',count:10},{key:'easy',label:'Facile',count:18},{key:'medium',label:'Moyen',count:100},{key:'hard',label:'Difficile',count:250}], flagQuiz:'Quiz des Drapeaux',flagSub:'Un drapeau apparaît — tape le nom du pays.',flagPlaceholder:'Entrer le pays …',flagWorld:'Monde entier', flagDiffs:[{key:'beginner',label:'Débutant'},{key:'easy',label:'Facile'},{key:'medium',label:'Moyen'},{key:'hard',label:'Difficile'}], homeTitle:'Jeux de Géographie',homeSub:'Teste tes connaissances du monde — pays, lacs, fleuves et drapeaux.', playBtn:'Jouer',optionsLbl:'Options',homeBtn:'← Accueil',scrollHint:'Faire défiler pour en voir plus',mapQuiz:'Carte du Monde',load:'Chargement de la carte …',back:'← Menu', foundLbl:'trouvés',correctLbl:'corrects',wrongLbl:'faux',remLbl:'restants',resTitle:'Quiz terminé !',again:'Rejouer',newgame:'Retour à la sélection', shareBtn:'📤 Partager',shareCopied:'Copié dans le presse-papiers !',shareTitle:'Quiz de la Carte du Monde', randomChallengeBtn:'🎲 Défi Aléatoire',dailyChallengeBtn:'📅 Défi du Jour',dailyLoginRequired:'Connecte-toi pour jouer au Défi du Jour',timeLbl:'Temps',dailyDoneTitle:'Déjà joué aujourd’hui !',dailyLeaderboardTitle:'Classement du jour',dailyHintTitle:'Attention, chrono lancé !',dailyHintText:'Le Défi du jour est chronométré — plus tu termines vite, meilleur sera ton classement du jour.',dailyHintDontShow:'Ne plus afficher',dailyHintBtn:'C’est parti !', langLbl:'Langue',themeLbl:'Thème',colorblindLabel:'Mode daltonien',colorblindTip:'Remplace le vert/rouge des réponses correctes/incorrectes par des couleurs plus distinguables, quel que soit le thème choisi.',colorblindOn:'Activé',colorblindOff:'Désactivé', projLbl:'Vue de la carte',projStd:'Natural Earth',projMerc:'Mercator', foundModeLabel:'Bonnes réponses',keepOn:'Reste en vert',keepOff:'S’estompe', wrongHintLabel:'Indices en cas d’erreur',wrongHintTip:'Après une mauvaise réponse, le pays correct est brièvement affiché.',wrongHintOn:'Afficher',wrongHintOff:'Masquer', skipHintLabel:'Indices en cas de passage',skipHintTip:'Affiche brièvement la cible passée après l’avoir passée.',skipHintOn:'Afficher',skipHintOff:'Masquer', countries:'pays',zoomTip:'Zoom et déplacement possibles',skipLbl:'Passer',skippedLbl:'passés', correctFb:n=>'✓ Correct ! '+n,wrongFb:n=>'C’était '+n, res1:(a,b,c)=>`${a} sur ${b} du premier coup (${c} %)`,res2:(a,b)=>`${a} corrects, ${b} manqués`,challengeScore:n=>'Score : '+n, themes:{atlas:'Atlas',neon:'Cyberpunk',terrain:'Terrain'}, cback:'← Retour',ctitle:'Mode Personnalisé',clblCont:'Continents',clblCount:'Nombre de pays',clblOf:'disponibles',clblAll:'Tous les pays',clblCountries:'Choisir les pays', cbtnAll:'Tous',cbtnNone:'Aucun',cbtnStart:'Démarrer', signIn:'Connexion',signUp:'Inscription',signOut:'Déconnexion',loginTitle:'Connexion',registerTitle:'Créer un compte', authEmail:'E-mail',authPw:'Mot de passe',authName:'Nom affiché (facultatif)',loggedIn:'Connecté en tant que', toRegister:'Pas de compte ? S’inscrire',toLogin:'Déjà un compte ? Se connecter', authFillAll:'Veuillez saisir l’e-mail et le mot de passe.',authNotConfigured:'Firebase n’est pas encore configuré.', authErrEmail:'Adresse e-mail invalide.',authErrInUse:'Cet e-mail est déjà utilisé.',authErrWeak:'Mot de passe trop faible (min. 6 caractères).', authErrCred:'E-mail ou mot de passe incorrect.',authErrGeneric:'Un problème est survenu. Réessaie.', authUsername:'Nom d’utilisateur',showPw:'Afficher le mot de passe',forgotPw:'Mot de passe oublié ?',resetSent:'E-mail de réinitialisation envoyé.', enterEmailFirst:'Saisis d’abord ton e-mail.',authUserRequired:'Choisis un nom d’utilisateur.',authUserInvalid:'3–20 caractères : lettres, chiffres, _',authUserTaken:'Nom d’utilisateur déjà pris.', googleBtn:'Se connecter avec Google',orSep:'ou',newRecord:'Nouveau record !',bestLabel:'Meilleur score', roundLimitLbl:'Nombre de cibles',allTargetsLbl:'Toutes les cibles',normalMode:'Mode normal', learnModeLbl:'Mode apprentissage',learnModeLogin:'Connecte-toi pour le Mode apprentissage',learnModeDesc:'Les erreurs fréquentes apparaissent en premier. Les sujets déjà maîtrisés 3 fois sont masqués.',learnModeCta:'→ Cliquer pour se connecter', inputQuiz:'Devine le pays',inputSub:'Drapeau, capitale ou silhouette — tape le bon nom.',flagCardSub:'Quel est ce drapeau ?', capitalQuiz:'Quiz des Capitales',capitalCardSub:'Pays ↔ capitale',outlineQuiz:'Quiz des Silhouettes',outlineCardSub:'Reconnais la forme', languageQuiz:'Quiz des Langues',languageCardSub:'Quelle est cette langue ?',currencyQuiz:'Quiz des Monnaies',currencyCardSub:'Quelle est cette monnaie ?', wappenQuiz:'Quiz des Blasons',wappenCardSub:'Quel est ce blason ?',neighborQuiz:'Énigme des Voisins',neighborCardSub:'Quel pays est bordé par ceux-ci ?', iqAskLanguage:'Quelle est cette langue ?',iqAskCurrency:'Quelle est cette monnaie ?',iqAskNeighbor:'Quel pays est bordé par ces pays ?', langPlaceholder:'Entrer la langue …',curPlaceholder:'Entrer la monnaie …',languages:'langues',currencies:'monnaies', cfgRegion:'Région',cfgDiff:'Difficulté',cfgDir:'Direction',cfgStart:'Démarrer le quiz', dirC2Cap:'Pays → capitale',dirCap2C:'Capitale → pays',dirFlag2Cap:'Drapeau → capitale',diffAll:'Tous',regWorld:'Monde', iqAskCapital:'Quelle est la capitale ?',iqAskCountry:'Quel pays ?',capPlaceholder:'Entrer la capitale …', cfgDropdownLabel:'Suggestions',cfgDropdownOn:'Afficher',cfgDropdownOff:'Masquer', cfgSkipHintLabel:'Indice au passage',cfgSkipHintOn:'Afficher',cfgSkipHintOff:'Masquer', regionQuiz:'Régions',regionSub:'Le nom d’une région apparaît — trouve-la et clique dessus sur la carte.',findRegion:'Trouve cette région',regions:'Régions', popQuiz:'Population',popSub:'Les pays les plus ou les moins peuplés — trouve-les et clique dessus sur la carte.',findPop:'Quel pays ?', popMostQ:'Le pays le plus peuplé du monde',popLeastQ:'Le pays le moins peuplé du monde', popMostRankQ:n=>n+'ᵉ pays le plus peuplé du monde',popLeastRankQ:n=>n+'ᵉ pays le moins peuplé du monde', areaQuiz:'Superficie',findArea:'Quel pays ?',findTrivia:'Quel pays ?',areaMostQ:'Le plus grand pays du monde par la superficie',areaLeastQ:'Le plus petit pays du monde par la superficie', areaMostRankQ:n=>n+'ᵉ plus grand pays du monde par la superficie',areaLeastRankQ:n=>n+'ᵉ plus petit pays du monde par la superficie',popMostNQ:n=>'Les '+n+' pays les plus peuplés du monde',popLeastNQ:n=>'Les '+n+' pays les moins peuplés du monde',areaMostNQ:n=>'Les '+n+' plus grands pays du monde par la superficie',areaLeastNQ:n=>'Les '+n+' plus petits pays du monde par la superficie',happyMostQ:'Le pays le plus heureux du monde',happyLeastQ:'Le pays le moins heureux du monde',happyMostRankQ:n=>n+'ᵉ pays le plus heureux du monde',happyLeastRankQ:n=>n+'ᵉ pays le moins heureux du monde',happyMostNQ:n=>'Les '+n+' pays les plus heureux du monde',happyLeastNQ:n=>'Les '+n+' pays les moins heureux du monde',exportMostQ:"Le pays avec le plus grand volume d'exportations au monde",exportLeastQ:"Le pays avec le plus petit volume d'exportations au monde",exportMostRankQ:n=>n+"ᵉ pays au monde par volume d'exportations",exportLeastRankQ:n=>n+"ᵉ en partant de la fin par volume d'exportations",exportMostNQ:n=>'Les '+n+" pays avec le plus grand volume d'exportations au monde",exportLeastNQ:n=>'Les '+n+" pays avec le plus petit volume d'exportations au monde",heritageMostQ:"Le pays avec le plus de sites du patrimoine mondial de l'UNESCO",heritageMostRankQ:n=>n+"ᵉ pour le plus de sites du patrimoine mondial de l'UNESCO",heritageMostNQ:n=>'Les '+n+" pays avec le plus de sites du patrimoine mondial de l'UNESCO",findTriviaN:'Quels pays ?', catHeading:'À quoi veux-tu jouer ?',catSub:'Choisis un type de jeu', catClickTitle:'Cliquer sur la carte',catClickSub:'Trouve pays, régions et curiosités sur la carte du monde', catPinTitle:'Plante le Pin',catPinSub:'Place le repère le plus près possible de l’emplacement', catInputTitle:'Texte et choix multiple',catInputSub:'Drapeaux, blasons, capitales et silhouettes', catOtherTitle:'Autres quiz de la carte du monde',catOtherSub:'Clique sur villes, fleuves, lacs et montagnes', clickRootHeading:'Cliquer sur la carte',clickRootSub:'Pays ou régions ?', landerTitle:'Pays',landerCardSub:'Carte du monde, continents, curiosités et plus',regionCardSub:'États, provinces et plus',landerRootSub:'Choisis un type de jeu', optWorld:'Carte du Monde',optWorldSub:'Les 197 pays',optContinents:'Continents',continentsSub:'Choisis un continent', optDifficulty:'Difficulté',difficultySub:'Pays triés par population',diffTierLabels:['Très facile','Facile','Moyen','Difficile','Très difficile'], comingSoon:'Bientôt disponible',optTrivia:'Curiosités',triviaSub:'Questions spéciales sur les pays', optCustom:'Mode Personnalisé',optCustomSub:'Choisis les continents et le nombre de pays', pinTriviaLbl:'Curiosités',landmarks:'Lieux',findPinTrivia:'Où se trouve ceci ?', perQuizLbl:'par quiz',wrongShort:'Faux !'}
};

let lang='de',theme='atlas',keepFound=true,showWrongHint=true,showSkipHint=true,showDropdown=true,projection='mercator',game={},quizRoundLimit=10,learnMode=false,allTargets=false,_challenge=null,dailyHintDismissed=false;
...
function renderClickPage(){
  ...
  const landerItems=[
    ...
    {icon:'💡',title:t('optTrivia'),sub:t('triviaSub'),onclick:'startTriviaGame()'},
...
const LANG_OPTIONS=[{code:'de',flag:'data/flags/h20/de.png',label:'Deutsch'},{code:'en',flag:'data/flags/h20/gb.png',label:'English'},{code:'es',flag:'data/flags/w320/es.png',label:'Español'},{code:'fr',flag:'data/flags/w320/fr.png',label:'Français'}];
function langGroup(){return og(t('langLbl'),
  LANG_OPTIONS.map(o=>`<button class="lb lb-flag${lang===o.code?' active':''}" onclick="setLang('${o.code}')"><img class="lbflag" src="${o.flag}" alt="">${o.label}</button>`).join(''));}
...
function colorblindGroup(){return og(t('colorblindLabel')+infoIcon(t('colorblindTip')),
  `<button class="lb${colorblindMode?' active':''}" onclick="setColorblindMode(true)">${t('colorblindOn')}</button>`+
  `<button class="lb${!colorblindMode?' active':''}" onclick="setColorblindMode(false)">${t('colorblindOff')}</button>`);}
...
function roundLimitGroup(){
  return og(t('roundLimitLbl'),
    ...
    `<span style="font-size:12px;opacity:.55;${allTargets?'opacity:.3':''}">${t('perQuizLbl')}</span>` +
...
function optHomeHTML(){return langGroup()+projGroup()+colorblindGroup()+wrongHintGroup()+skipHintGroup()+roundLimitGroup();}
...
function gatherSettings(){return {lang,theme,colorblindMode,keepFound,showWrongHint,showSkipHint,showDropdown,projection,learnMode,allTargets,quizRoundLimit,customConts:[...customConts],dailyHintDismissed};}
function applyRemoteSettings(s){
  if(!s)return;
  if(LANG_OPTIONS.some(o=>o.code===s.lang))lang=s.lang;
  if(s.theme&&THEMES[s.theme])theme=s.theme;
  if(typeof s.colorblindMode==='boolean'){colorblindMode=s.colorblindMode;_applyColorblindOverride();}
  ...
  if(typeof s.dailyHintDismissed==='boolean')dailyHintDismissed=s.dailyHintDismissed;
...
function gameScoreKey(){
  ...
  if(game.triviaMode)return 'trivia:world';
...
function gameScoreValue(){
  ...
  if(game.pinMode){const isCont=['EU','AF','AS','NA','SA','OC','world','trivia'].includes(game.pinRegion);return {score:game.pinScore||0,total:(game.total||0)*(isCont?1050:1000)};}
...
function renderHome(){
  ...
  setTxt('home-play',t('playBtn'));setTxt('home-random-btn',t('randomChallengeBtn'));setTxt('home-daily-btn',t('dailyChallengeBtn'));setTxt('home-options-btn','⚙ '+t('optionsLbl'));
  const dailyBtn=$('home-daily-btn');if(dailyBtn)dailyBtn.title=window._authUser?'':t('dailyLoginRequired');
...
function updateAllText(){
  ...
  setTxt('btn-back',t('back'));setTxt('find-label',game.riverMode?t('findRiver'):game.lakeMode?t('findLake'):game.cityMode?t('findCity'):game.mountainMode?t('findMountain'):game.rangeMode?t('findRange'):game.pinMode?(game.pinRegion==='trivia'?t('findPinTrivia'):t('findPin')):game.triviaMode?(game.currentGroup&&game.currentGroup.size>1?t('findTriviaN'):t('findTrivia')):t('find'));setTxt('found-label',t('foundLbl'));setTxt('lbl-c',t('correctLbl'));setTxt('lbl-w',t('wrongLbl'));setTxt('btn-skip',t('skipLbl'));setTxt('res-title',t('resTitle'));setTxt('btn-again',t('again'));setTxt('btn-new',t('newgame'));setTxt('btn-share',t('shareBtn'));setTxt('opts-title',t('optionsLbl'));setTxt('flag-btn-back',t('back'));setTxt('flag-lbl-c',t('correctLbl'));setTxt('flag-lbl-w',t('wrongLbl'));
  ...
  if(game.current&&C[game.current]&&!(game.riverMode||game.lakeMode||game.cityMode||game.mountainMode||game.rangeMode||game.pinMode||game.triviaMode))$('target-name').textContent=cn(game.current);updateStats();
...
function sectionInner(key){
  if(key==='map'){
    ...
    return ms.map(m=>gsCard(m==='custom'?'openCustom()':"startGame('"+m+"')",MODES[m][lang],m==='custom'?'⚙':MODES[m].cnt+' '+t('countries'),m==='custom'?null:'map:'+m)).join('')
      +gsCard('startTriviaGame()',t('optTrivia'),'💡','trivia:world');
  }
  ...
  if(key==='pin'){
    ...
    const trivia=gsCard("startPinGame('trivia')",t('pinTriviaLbl'),getPinCities('trivia').length+' '+t('landmarks'),'pin:trivia');
    return world+conts.join('')+'<div style="grid-column:1/-1;height:1px;background:rgba(226,90,90,.2);margin:.5rem 0;"></div>'+countries.join('')+'<div style="grid-column:1/-1;height:1px;background:rgba(226,90,90,.2);margin:.5rem 0;"></div>'+trivia;
  }
  if(key==='flag'){
...
const INPUT_QUIZZES=[
  ...
  {type:'neighbor',titleKey:'neighborQuiz',subKey:'neighborCardSub'}
];
// Nachbarland-Flaggen-Rätsel: guess the country from its neighbors' flags alone (e.g. Switzerland +
// Austria -> Liechtenstein). Curated from the map's own real adjacency (topojson.neighbors() on
// countries-50m.json, computed once in a browser console and pasted in — see Entscheidungen.md),
// restricted to countries with exactly 2 or 3 land neighbors so the puzzle stays tight and roughly
// as solvable as the Liechtenstein example. Two pairs were excluded entirely because they share an
// identical neighbor-set with another country (UAE/Yemen both border only Saudi Arabia+Oman;
// Bhutan/Nepal both border only India+China) — showing that pair would have no unique answer.
// Somalia was excluded too: the 50m topology doesn't carry its short border with Djibouti, so its
// computed neighbor list (Kenya+Ethiopia only) is geographically incomplete.
const NEIGHBOR_PUZZLES=[{id:8,neighbors:[499,807,300]},
  {id:20,neighbors:[724,250]},
  {id:226,neighbors:[266,120]},
  {id:50,neighbors:[356,104]},
  {id:84,neighbors:[484,320]},
  {id:70,neighbors:[688,499,191]},
  {id:72,neighbors:[716,710,516]},
  {id:108,neighbors:[834,646,180]},
  {id:152,neighbors:[604,68,32]},
  {id:188,neighbors:[591,558]},
  {id:262,neighbors:[231,232]},
  {id:218,neighbors:[604,170]},
  {id:222,neighbors:[340,320]},
  {id:232,neighbors:[729,231,262]},
  {id:233,neighbors:[643,428]},
  {id:748,neighbors:[710,508]},
  {id:246,neighbors:[752,643,578]},
  {id:266,neighbors:[226,178,120]},
  {id:288,neighbors:[768,384,854]},
  {id:624,neighbors:[686,324]},
  {id:328,neighbors:[862,740,76]},
  {id:340,neighbors:[558,320,222]},
  {id:360,neighbors:[626,598,458]},
  {id:116,neighbors:[704,764,418]},
  {id:414,neighbors:[682,368]},
  {id:422,neighbors:[760,376]},
  {id:430,neighbors:[694,324,384]},
  {id:438,neighbors:[756,40]},
  {id:442,neighbors:[276,250,56]},
  {id:454,neighbors:[894,834,508]},
  {id:458,neighbors:[764,360,96]},
  {id:504,neighbors:[12,478]},
  {id:484,neighbors:[840,320,84]},
  {id:498,neighbors:[804,642]},
  {id:496,neighbors:[643,156]},
  {id:558,neighbors:[340,188]},
  {id:528,neighbors:[276,56]},
  {id:408,neighbors:[410,643,156]},
  {id:578,neighbors:[752,643,246]},
  {id:512,neighbors:[887,784,682]},
  {id:275,neighbors:[400,376,818]},
  {id:591,neighbors:[188,170]},
  {id:600,neighbors:[76,68,32]},
  {id:752,neighbors:[578,246]},
  {id:694,neighbors:[430,324]},
  {id:724,neighbors:[620,250,20]},
  {id:740,neighbors:[328,250,76]},
  {id:768,neighbors:[288,854,204]},
  {id:788,neighbors:[434,12]},
  {id:858,neighbors:[76,32]},
  {id:840,neighbors:[484,124]},
  {id:862,neighbors:[328,170,76]},
  {id:704,neighbors:[418,156,116]}
];
const NEIGHBOR_PUZZLE_MAP=new Map(NEIGHBOR_PUZZLES.map(p=>[p.id,p.neighbors]));
...
function back(){if(_regGame)return void regionBack();if(game.challengeMode){_stopDailyTimer();game={};_challenge=null;goHome();return;}const key=game.flagMode?'flag':game.pinMode?'pin':game.cityMode?'city':game.riverMode?'river':game.lakeMode?'lake':(game.mountainMode||game.rangeMode)?'mountain':null;game={};if(key){goToGames(key);return;}_quizBackAction();}
...
function skip(){
  ...
  // Challenge mode (Random/Daily): a skip is scored exactly like a wrong answer — 0 points for
  // this question — by reusing advanceChallenge()'s existing "only count wrongOnCurrent===false as
  // a first-try hit" check, rather than adding a second, parallel scoring path.
  if(game.challengeMode){game.wrongOnCurrent=true;_challenge.skipped=(_challenge.skipped||0)+1;nextCountry();return;}
  game.skipped=(game.skipped||0)+1; // one skip, even for a multi-country question — see currentGroup below
  if(!game.skippedItems)game.skippedItems=new Set();
  if(game.currentGroup){
    game.currentGroup.forEach(id=>game.skippedItems.add(id));
    game.currentGroup=null;
  }else{
    game.skippedItems.add(game.current);
  }
  clearFeedback();if(showSkipHint)updateColors();updateStats();nextCountry();
}
...
function resolveAnswer({key,correct,name,flashKey,flash,repaint,keepOnFound=true}){
  if(correct){
    ...
    // Multi-country trivia question, and other members are still unfound: stay on this question —
    // no lock, no advance, no firstTry credit yet (that's only earned once the whole question, not
    // just one of its countries, is done). Every other mode never sets currentGroup, so this branch
    // never runs for them and behaviour is unchanged.
    if(game.currentGroup){
      game.currentGroup.delete(key);
      if(game.currentGroup.size>0){
    ...
    game.questionsCompleted=(game.questionsCompleted||0)+1;
    if(showWrongHint)showFeedback(t('correctFb')(name),THEMES[theme].found);
    recordCorrectForCurrent();repaint();updateStats();
...
function lakeDisplayName(feat){const p=feat.properties;return p['name_'+lang]||p.name_en||p.name_de||p.name||'?';}
...
function riverDisplayName(feat){
  ...
  return p['name_'+lang]||p.name_en||p.name_de||p.name||'?';
}
...
// ── CITY QUIZ ──
function cityDisplayName(c){return c['name_'+lang]||c.name_en||c.name_de||'?';}
...
function mountainDisplayName(m){return m['name_'+lang]||m.name_en||m.name_de||'?';}
...
function rangeDisplayName(feat){const p=feat.properties;return p['name_'+lang]||p.name_en||p.name_de||'?';}
...
function handlePinClick(ev){
  ...
  const isContinent=['EU','AF','AS','NA','SA','OC','world','trivia'].includes(game.pinRegion);
...
function showPinResult(){
  ...
  const isCont=['EU','AF','AS','NA','SA','OC','world','trivia'].includes(game.pinRegion);
  ...
  $('res-btn-back').textContent=t('back');$('btn-again').textContent=t('again');$('btn-new').textContent=t('newgame');$('btn-share').textContent=t('shareBtn');
  if(typeof checkAchievements==='function')checkAchievements();
}

// ── TRIVIA QUIZ ──
// One quiz mixing rank-based questions across five metrics (population, area, happiness, exports,
// UNESCO heritage sites) — reuses the plain country quiz's click handler and colouring entirely
// unchanged: every target is a real country id, so isActive()/getColor() already do the right
// thing for a world-scoped round. What this mode adds: how the round queue is built (multiple
// metrics, single-country AND multi-country "top N" questions mixed together) and what question
...
// The map's own topojson geometry already IS the area data, via d3.geoArea() (steradians,
// converted to km² by the sphere's total surface area) — no extra data/ file, no CACHE_NAME bump.
// EXCEPT: every country in MICROSTATES (countries.js) is rendered as a dot, not a real polygon
// — it has no feature in the topology at all, so the loop below silently never assigns it an
// area. That's not a rounding quirk: it's the entire bottom of the world's area ranking (Vatican
// City, Monaco, Nauru, Tuvalu, San Marino, ...), so a "5 smallest countries" trivia question was
// missing exactly the 5 real smallest countries. Real-world km² for all 36, hand-verified via web
// search (Wikipedia/World Atlas country infoboxes, land-area figures), merged in as a fallback.
const MICROSTATE_AREA_KM2={492:2.02,336:0.49,674:61.2,20:468,438:160,48:786.8,702:728,462:298,
  296:811,520:21,798:26,584:181,583:702,585:459,882:2842,776:747,690:455,480:2040,174:1861,
  678:964,132:4033,52:430,308:344,662:616,659:261,670:389,28:442,212:751,470:316,442:2586,
  780:5127,548:12189,90:28896,270:11295,388:10991,242:18274};
let areaData=null;
function _computeAreaData(){
  if(areaData)return areaData;
  if(!worldData)return null;
  const EARTH_KM2=510072000; // total surface area of a sphere with Earth's mean radius (6371 km)
  const d={};
  // A few countries (e.g. Australia, id 36) appear as more than one feature in the topology —
  // an outlying island kept as its own polygon instead of folded into the main multipolygon.
  // Sum rather than overwrite, or the last-processed sliver would replace the real area.
  topojson.feature(worldData,worldData.objects.countries).features.forEach(f=>{
    const id=+f.id;if(id<0)return;
    d[id]=(d[id]||0)+d3.geoArea(f)*EARTH_KM2/(4*Math.PI);
  });
  for(const id in MICROSTATE_AREA_KM2){if(d[id]==null)d[id]=MICROSTATE_AREA_KM2[id];}
  areaData=d;
  return d;
}
let happinessData=null;
async function _loadHappinessData(){
  if(happinessData)return happinessData;
  happinessData=await fetch('data/happiness.json').then(r=>r.json()).catch(()=>({}));
  return happinessData;
}
let exportsData=null;
async function _loadExportsData(){
  if(exportsData)return exportsData;
  exportsData=await fetch('data/exports.json').then(r=>r.json()).catch(()=>({}));
  return exportsData;
}
let heritageData=null;
async function _loadHeritageData(){
  if(heritageData)return heritageData;
  heritageData=await fetch('data/heritage.json').then(r=>r.json()).catch(()=>({}));
  return heritageData;
}
function _popQuestionText(rank,dir){return rank===1?(dir==='most'?t('popMostQ'):t('popLeastQ')):(dir==='most'?t('popMostRankQ')(rank):t('popLeastRankQ')(rank));}
function _popMultiQuestionText(n,dir){return dir==='most'?t('popMostNQ')(n):t('popLeastNQ')(n);}
function _areaQuestionText(rank,dir){return rank===1?(dir==='most'?t('areaMostQ'):t('areaLeastQ')):(dir==='most'?t('areaMostRankQ')(rank):t('areaLeastRankQ')(rank));}
function _areaMultiQuestionText(n,dir){return dir==='most'?t('areaMostNQ')(n):t('areaLeastNQ')(n);}
function _happyQuestionText(rank,dir){return rank===1?(dir==='most'?t('happyMostQ'):t('happyLeastQ')):(dir==='most'?t('happyMostRankQ')(rank):t('happyLeastRankQ')(rank));}
function _happyMultiQuestionText(n,dir){return dir==='most'?t('happyMostNQ')(n):t('happyLeastNQ')(n);}
function _exportQuestionText(rank,dir){return rank===1?(dir==='most'?t('exportMostQ'):t('exportLeastQ')):(dir==='most'?t('exportMostRankQ')(rank):t('exportLeastRankQ')(rank));}
function _exportMultiQuestionText(n,dir){return dir==='most'?t('exportMostNQ')(n):t('exportLeastNQ')(n);}
// Heritage-site counts have a long tail of ties (dozens of countries with exactly 1 site, plus
// every country not in data/heritage.json at all effectively tied at 0) — a "least" ranking there
// would mostly be arbitrary tie-breaking, not real trivia, so this metric only ever asks "most".
...
const TRIVIA_SINGLE_PER_SIDE=2; // individual rank questions per metric+direction
const TRIVIA_SINGLE_MAX_RANK=8; // single-question ranks never go deeper than this — "Platz 17" is too obscure to guess
const TRIVIA_GROUP_MIN=3,TRIVIA_GROUP_MAX=5; // "top N" question size — matches "the 3/4/5 countries with..."
async function startTriviaGame(){
  if(!worldData)await loadMap();
  const [pop,exportsD,happyD,heritageD]=await Promise.all([_loadPopData(),_loadExportsData(),_loadHappinessData(),_loadHeritageData()]);
  const area=_computeAreaData();
  const sortFor=data=>Object.keys(C).map(Number).filter(id=>data[id]!=null).sort((a,b)=>data[b]-data[a]);
  const metrics=[
    {sorted:sortFor(pop),dirs:['most','least'],singleQ:_popQuestionText,multiQ:_popMultiQuestionText},
    {sorted:sortFor(area),dirs:['most','least'],singleQ:_areaQuestionText,multiQ:_areaMultiQuestionText},
    {sorted:sortFor(happyD),dirs:['most','least'],singleQ:_happyQuestionText,multiQ:_happyMultiQuestionText},
  ...
  game={mode:'trivia',triviaMode:true,
    queue:shuffled,current:null,currentGroup:null,
    found:new Set(),correct:0,wrong:0,firstTry:0,questionsCompleted:0,
    total:shuffled.length,wrongOnCurrent:false,skipped:0};
  lastMode='trivia';canClick=true;optsOpen=false;
  showScreen('game-screen');$('opts-panel').style.display='none';
  $('target-name').textContent='';showFeedback(t('load'),'#888');
  updateAllText();
  renderMap(worldData);
...
function isActive(id){
  ...
  if(game.pinMode){
    ...
    if(!r||r==='world'||r==='trivia')return !!C[id];
...
// ── RANDOM CHALLENGE ──
// Mixes question types from the existing map-click (country) and text-input (flag/capital/
// outline/language/currency/wappen) quizzes into one 10-question round. Deliberately does not
// touch handleClick/resolveAnswer/submitFlag at all — each question is set up as if it were an
// ordinary single-target instance of its own quiz type (reusing all their existing rendering
// and answer-checking), and the *only* hook is at the "this question is done, what's next"
// boundary each of those already has (nextCountry for map clicks, nextFlag's empty-queue check
// for text input) — redirected to advanceChallenge() instead of their normal "next same-type
// question or show final result" behaviour. A wrong answer doesn't advance, same as every other
// quiz in the app — retry until correct. Not currently included: river/lake/city/mountain/pin,
// which have their own distinct pools/mechanics and would need their own integration work.
const CHALLENGE_TYPES=['country','flag','capital','outline','language','currency','wappen'];
// flag/wappen questions each need their own per-country image, fetched lazily by nextFlag()'s
// <img src=...> otherwise — fine for the normal flag quiz (one image at a time, no round-trip
// timer running), but in a challenge that meant a visible loading flicker mid-round, and for the
// Daily Challenge specifically, silently counted that download time against the player's timed
// score. Fetching every image this round will need up front, before the timer/round starts,
// fixes both. img.onerror also resolves (not rejects) so one missing/offline image can't hang the
// whole round — nextFlag() already renders whatever the browser ends up with regardless.
function _preloadChallengeImages(queue){
  const urls=[];
  queue.forEach(q=>{
    if(q.type==='flag')urls.push('data/flags/w320/'+ISO2[q.id]+'.png');
    else if(q.type==='wappen')urls.push('data/wappen/'+q.id+'.png');
  });
...
async function startRandomChallenge(){
  ...
  const pools={
    country:countryIds,
    flag:countryIds.filter(id=>ISO2[id]),
    capital:countryIds.filter(id=>CAPITALS[id]),
    outline:countryIds.filter(id=>!OUTLINE_EXCLUDE.has(id)&&iqOutlineFeature(id)),
    language:Object.keys(LANGUAGES),
    currency:Object.keys(CURRENCIES),
    wappen:countryIds.filter(id=>!WAPPEN_EXCLUDE.has(id))
  };
  const queue=[];
  for(let i=0;i<10;i++){
    const type=CHALLENGE_TYPES[Math.floor(Math.random()*CHALLENGE_TYPES.length)];
    const pool=pools[type];
    queue.push({type,id:pool[Math.floor(Math.random()*pool.length)]});
  }
  await _preloadChallengeImages(queue);
  _challenge={queue,pos:0,firstTry:0,total:10,startTs:Date.now()};
  nextChallengeQuestion();
}
// One combined number instead of two separate stats (right answers, elapsed time) — 100 points per
// first-try-correct question, minus one point per elapsed second, floored at 0 so a slow-but-clean
// run never scores negative. A skipped question contributes nothing (see skip()/skipFlag(), which
// mark it exactly like a wrong answer so it's excluded from firstTry), matching "skipped questions
// give 0 points". Shared by Random and Daily Challenge — both set startTs at launch.
function _challengeScore(g){return Math.max(0,g.firstTry*100-Math.floor((Date.now()-g.startTs)/1000));}
function nextChallengeQuestion(){
  if(!_challenge)return;
  if(_challenge.pos>=_challenge.total){showChallengeResult();return;}
  const q=_challenge.queue[_challenge.pos];
  if(q.type==='country'){
    game={challengeMode:true,current:q.id,found:new Set(),correct:0,wrong:0,wrongOnCurrent:false};
    canClick=true;
    showScreen('game-screen');
    if(!countryPaths)renderMap(worldData);
    $('find-label').textContent=t('find');
    $('target-name').textContent=cn(q.id);
    clearFeedback();updateColors();updateStats();
  }else{
    game={challengeMode:true,flagMode:true,inputMode:q.type,capitalDir:'c2cap',flagQueue:[q.id],flagCorrect:0,flagWrong:0,flagFound:new Set(),wrongOnCurrent:false};
    showScreen('flag-screen');
    nextFlag();
  }
}
function advanceChallenge(){
  if(!game.wrongOnCurrent)_challenge.firstTry++;
  _challenge.pos++;
  nextChallengeQuestion();
}
function showChallengeResult(){
  const g=_challenge,p=Math.round((g.firstTry/g.total)*100);
  g.score=_challengeScore(g);
  if(g.daily)_stopDailyTimer();
  showScreen('result-screen');
  $('res-emoji').textContent=p>=90?'🏆':p>=70?'🎉':p>=50?'👍':'📚';
  $('res-title').textContent=t('resTitle');
  $('res-l1').textContent=t('res1')(g.firstTry,g.total,p);
  $('res-l2').textContent=t('challengeScore')(g.score);
  $('res-btn-back').textContent=t('back');$('btn-again').style.display='';$('btn-again').textContent=t('again');$('btn-new').textContent=t('newgame');$('btn-share').textContent=t('shareBtn');
  const dw=$('daily-leaderboard-wrap');
  if(g.daily){
    g.timeMs=Date.now()-g.startTs;
    $('res-l2').textContent=t('timeLbl')+': '+_formatDailyTime(g.timeMs)+' · '+t('challengeScore')(g.score);
    $('btn-again').style.display='none'; // once-per-day: no replay
    if(dw){dw.style.display='';$('daily-leaderboard-title').textContent=t('dailyLeaderboardTitle');}
    if(typeof submitDailyChallengeScore==='function'){
      submitDailyChallengeScore(g.date,g.firstTry,g.total,g.timeMs,g.score).then(()=>renderDailyLeaderboard(g.date));
    }
  }else if(dw)dw.style.display='none';
  // game.challengeMode deliberately stays set (not reset to {}) so restart()/back() can still
  // tell this result screen belongs to a challenge — they're the ones that clear it.
  if(typeof checkAchievements==='function')checkAchievements();
}

// ── DAILY CHALLENGE ──
// Same mechanic as Random Challenge above (game.challengeMode, the shared "next question" hooks
// in nextCountry()/nextFlag()) — the only difference is the question queue is built with a PRNG
// seeded from today's date string instead of Math.random(), so every signed-in player gets the
// exact same 10 questions on the same calendar day (deterministic pool order: Object.keys(C) is
// spec-guaranteed ascending for its numeric-string keys, so it's identical across every browser).
// _challenge gains daily/date/startTs/timeMs fields that only showChallengeResult() above branches
// on — nextChallengeQuestion() itself is untouched and fully shared with Random Challenge.
...
function _dailyToday(){return typeof _todayStr==='function'?_todayStr():new Date().toISOString().slice(0,10);}
function _formatDailyTime(ms){return(Math.max(0,ms)/1000).toFixed(1)+'s';}
let _dailyTimerHandle=null;
function _startDailyTimer(){_stopDailyTimer();_dailyTimerHandle=setInterval(()=>{if(_challenge&&_challenge.daily){updateStats();updateFlagStats();}},500);}
function _stopDailyTimer(){if(_dailyTimerHandle){clearInterval(_dailyTimerHandle);_dailyTimerHandle=null;}}

async function openDailyChallenge(){
  if(!window._authUser){openAuth('login');return;}
  const today=_dailyToday();
  const already=(typeof getDailyChallengeResult==='function')?await getDailyChallengeResult(today):null;
  if(already){showDailyAlreadyPlayed(already);return;}
  if(!dailyHintDismissed){_showDailyHint(today);return;}
  await startDailyChallenge(today);
}
let _dailyHintPendingDate=null;
function _showDailyHint(dateStr){
  _dailyHintPendingDate=dateStr;
  $('daily-hint-title').textContent=t('dailyHintTitle');
  $('daily-hint-text').textContent=t('dailyHintText');
  $('daily-hint-dontshow-lbl').textContent=t('dailyHintDontShow');
  $('daily-hint-dontshow').checked=false;
  $('daily-hint-btn').textContent=t('dailyHintBtn');
  $('daily-hint-modal').style.display='flex';
}
function _confirmDailyHint(){
  if($('daily-hint-dontshow').checked){dailyHintDismissed=true;persistSettings();}
  $('daily-hint-modal').style.display='none';
  const dateStr=_dailyHintPendingDate;_dailyHintPendingDate=null;
  if(dateStr)startDailyChallenge(dateStr);
}
async function startDailyChallenge(dateStr){
  if(!outlineGeo)await iqEnsureOutlineGeo();
  if(!worldData)await loadMap();
  const rng=_seededRng(dateStr);
  const countryIds=Object.keys(C).map(Number);
  const pools={
    country:countryIds,
    flag:countryIds.filter(id=>ISO2[id]),
  ...
  for(let i=0;i<DAILY_ROUNDS;i++){
    ...
    const pool=pools[type];
    queue.push({type,id:pool[Math.floor(rng()*pool.length)]});
  }
  // Preloaded (and startTs/the timer only starting afterwards) so a slow connection's download
  // time for flag/wappen images never counts against the player's timed score — see
  // _preloadChallengeImages()'s comment.
  await _preloadChallengeImages(queue);
  _challenge={queue,pos:0,firstTry:0,total:DAILY_ROUNDS,daily:true,date:dateStr,startTs:Date.now()};
  _startDailyTimer();
  nextChallengeQuestion();
}
function showDailyAlreadyPlayed(result){
  game={challengeMode:true};
  _challenge={daily:true,date:result.date,firstTry:result.firstTry,total:result.total,timeMs:result.timeMs};
  showScreen('result-screen');
  const p=Math.round((result.firstTry/result.total)*100);
  $('res-emoji').textContent=p>=90?'🏆':p>=70?'🎉':p>=50?'👍':'📚';
  $('res-title').textContent=t('dailyDoneTitle');
  $('res-l1').textContent=t('res1')(result.firstTry,result.total,p);
  $('res-l2').textContent=t('timeLbl')+': '+_formatDailyTime(result.timeMs);
  $('res-btn-back').textContent=t('back');$('btn-new').textContent=t('newgame');$('btn-share').textContent=t('shareBtn');
  $('btn-again').style.display='none';
  const dw=$('daily-leaderboard-wrap');
  if(dw){dw.style.display='';$('daily-leaderboard-title').textContent=t('dailyLeaderboardTitle');}
  renderDailyLeaderboard(result.date);
}
async function renderDailyLeaderboard(dateStr){
  const el=$('daily-leaderboard-list');
  if(!el)return;
  el.innerHTML='<div style="text-align:center;color:#888;padding:1rem;font-size:13px;">'+t('load')+'</div>';
  const rows=(typeof getDailyLeaderboard==='function')?await getDailyLeaderboard(dateStr,20):[];
  if(!rows.length){el.innerHTML='<div style="text-align:center;color:#888;padding:1rem;font-size:13px;">–</div>';return;}
  const myUid=window._authUser&&window._authUser.uid;
  el.innerHTML=rows.map((r,i)=>{
    const mine=r.uid===myUid;
    return '<div style="display:flex;align-items:center;gap:10px;padding:6px 10px;border-radius:6px;'+(mine?'background:rgba(29,158,117,0.15);':'')+'">'+
      '<span style="width:22px;color:#888;font-size:13px;">'+(i+1)+'</span>'+
      '<span style="flex:1;font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+escapeHtml(r.username||'?')+'</span>'+
      '<span style="font-size:13px;color:#aaa;">'+r.firstTry+'/'+r.total+'</span>'+
      '<span style="font-size:13px;color:#aaa;width:52px;text-align:right;">'+_formatDailyTime(r.timeMs)+'</span>'+
      '</div>';
  }).join('');
}
...
function nextCountry(){
  if(game.challengeMode){advanceChallenge();return;}
  canClick=true;
  if(game.queue&&game.found){while(game.queue.length&&_queueEntryResolved(game.queue[0]))game.queue.shift();}
  if(!game.queue||game.queue.length===0){showResult();return;}
  const entry=game.queue.shift();
  if(entry&&typeof entry==='object'&&entry.ids){
    game.current=entry.ids[0];
    game.currentGroup=new Set(entry.ids);
    game._triviaQ=entry.q;
  }else{
    game.current=entry;
    game.currentGroup=null;
    game._triviaQ=null;
  }
  game.wrongOnCurrent=false;
  if(game.riverMode){const f=game.riverFeatures[game.current];$('target-name').textContent=f?riverDisplayName(f):'?';}
  else if(game.lakeMode){const f=game.lakeFeatures[game.current];$('target-name').textContent=f?lakeDisplayName(f):'?';}
  else if(game.cityMode){const c=game.cityFeatures[game.current];$('target-name').textContent=c?cityDisplayName(c):'?';}
  else if(game.mountainMode){const m=game.mountainFeatures[game.current];$('target-name').textContent=m?mountainDisplayName(m):'?';}
  else if(game.rangeMode){const r=game.rangeFeatures[game.current];$('target-name').textContent=r?rangeDisplayName(r):'?';}
  else if(game.triviaMode){$('target-name').textContent=game._triviaQ||'?';$('find-label').textContent=game.currentGroup&&game.currentGroup.size>1?t('findTriviaN'):t('findTrivia');}
  else{$('target-name').textContent=cn(game.current);}
  clearFeedback();updateStats();updateColors();
}
...
function updateStats(){
  if(game.challengeMode){
    ...
    $('score-disp').textContent=g.pos+'/'+g.total;$('found-label').textContent=t('foundLbl');
    $('stat-c').textContent=g.firstTry;$('lbl-c').textContent=t('correctLbl');
    $('stat-w').textContent=game.wrong||0;$('lbl-w').textContent=t('wrongLbl');
    if(g.daily){$('stat-s').textContent=_formatDailyTime(Date.now()-g.startTs);$('lbl-s').textContent=t('timeLbl');}
    else{$('stat-s').textContent=g.skipped||0;$('lbl-s').textContent=t('skippedLbl');}
    $('stat-r').textContent=(g.total-g.pos)+' '+t('remLbl');
    $('prog-bar').style.width=Math.round((g.pos/g.total)*100)+'%';
    return;
  }
  ...
  if(game.triviaMode){
    // game.found tracks individual countries, not questions — a "top 5" question alone adds 5 —
    // so the found/total ratio needs its own per-question counter instead of game.found.size,
    // or it could show more "found" than the round's own total.
    const tot=game.total||1,dn=game.questionsCompleted||0,rm=(game.queue?game.queue.length:0)+(game.current?1:0);
    $('score-disp').textContent=dn+'/'+tot;$('found-label').textContent=t('foundLbl');
    $('stat-c').textContent=game.correct||0;$('lbl-c').textContent=t('correctLbl');
    $('stat-w').textContent=game.wrong||0;$('lbl-w').textContent=t('wrongLbl');
    $('stat-s').textContent=game.skipped||0;$('lbl-s').textContent=t('skippedLbl');
...
function showResult(){if(!game.total){back();return;}const p=game.total>0?Math.round((game.firstTry/game.total)*100):0;const note=resultBestNote();game.current=null;showScreen('result-screen');$('res-btn-back').textContent=t('back');$('res-emoji').textContent=p>=90?'🏆':p>=70?'🎉':p>=50?'👍':'📚';$('res-title').textContent=t('resTitle');$('res-l1').textContent=t('res1')(game.firstTry,game.total,p);const sk=game.skipped||0;$('res-l2').textContent=t('res2')(game.correct,game.wrong)+(sk>0?', '+sk+' '+t('skippedLbl'):'')+note;$('btn-again').textContent=t('again');$('btn-new').textContent=t('newgame');$('btn-share').textContent=t('shareBtn');if(typeof checkAchievements==='function')checkAchievements();}

// Reads the already-rendered result-screen text rather than re-deriving it from game state,
// so this works unchanged for every mode (plain quiz, pin, region, flag/capital/outline …) —
// they all populate the same res-emoji/res-title/res-l1/res-l2 elements before this can be
// clicked. Falls back to clipboard only when the Web Share API is unavailable; if the user
// cancels a native share sheet, that's a "no", not a cue to silently copy instead.
async function shareResult(){
  const text=[$('res-emoji').textContent,$('res-title').textContent,$('res-l1').textContent,$('res-l2').textContent].filter(Boolean).join(' ');
  if(navigator.share){
    try{await navigator.share({title:t('shareTitle'),text,url:location.href});}catch(e){}
    return;
  }
  try{
    await navigator.clipboard.writeText(text+'\n'+location.href);
    const btn=$('btn-share'),old=btn.textContent;
...
const IQ_PREFIX={flag:'flag:',capital:'cap:',outline:'out:',language:'lang:',currency:'cur:',wappen:'coa:',neighbor:'nbr:'};
...
function iqPoolIds(cfg){
  ...
  if(cfg.type==='neighbor')return NEIGHBOR_PUZZLES.map(p=>p.id);
  let base;
  if(cfg.type==='flag')base=Object.keys(C).map(Number).filter(id=>ISO2[id]);
  else if(cfg.type==='capital')base=Object.keys(C).map(Number).filter(id=>CAPITALS[id]&&(cfg.dir!=='flag2cap'||ISO2[id]));
...
function iqAnswerCapital(){return game.inputMode==='capital'&&(game.capitalDir==='c2cap'||game.capitalDir==='flag2cap');}
function iqDisplayName(id){
  const mode=game.inputMode;
  if(mode==='language')return LANGUAGES[id][lang]||LANGUAGES[id].de;
  if(mode==='currency')return CURRENCIES[id][lang]||CURRENCIES[id].de;
  return iqAnswerCapital()?(CAPITALS[id][lang]||CAPITALS[id].de):(C[id][lang]||C[id].de);
}
...
function renderInputCfg(){
  const meta={flag:{title:t('flagQuiz'),ic:'🚩'},capital:{title:t('capitalQuiz'),ic:'🏛️'},outline:{title:t('outlineQuiz'),ic:'🗺️'},language:{title:t('languageQuiz'),ic:'🗣️'},currency:{title:t('currencyQuiz'),ic:'💱'},wappen:{title:t('wappenQuiz'),ic:'🛡️'},neighbor:{title:t('neighborQuiz'),ic:'🧩'}}[iqCfg.type];
  ...
  let html='';
  // Language/currency/neighbor pools aren't scoped by continent or difficulty (see iqPoolIds) —
  // skip those chips entirely rather than showing controls that don't do anything.
  const scopable=iqCfg.type!=='language'&&iqCfg.type!=='currency'&&iqCfg.type!=='neighbor';
  if(iqCfg.type==='capital'){
    html+=`<div class="custom-card"><div class="custom-label">${t('cfgDir')}</div><div class="iq-chips">`+
      ...
      chip(iqCfg.dir==='cap2c',"setIqCfg('dir','cap2c')",t('dirCap2C'))+
      chip(iqCfg.dir==='flag2cap',"setIqCfg('dir','flag2cap')",t('dirFlag2Cap'))+`</div></div>`;
  }
  if(scopable){
...
function skipFlag(){
  ...
  // Challenge mode (Random/Daily): see skip()'s comment — a skip must not count as a first-try hit.
  if(game.challengeMode){game.wrongOnCurrent=true;_challenge.skipped=(_challenge.skipped||0)+1;nextFlag();return;}
...
function nextFlag(){
  if(!game.flagQueue||game.flagQueue.length===0){if(game.challengeMode){advanceChallenge();return;}showFlagResult();return;}
  game.flagCurrent=game.flagQueue.shift();
  const id=game.flagCurrent,mode=game.inputMode||'flag';
  const img=$('flag-img'),txt=$('iq-text'),out=$('iq-outline-wrap'),nb=$('iq-neighbor-wrap');
  img.style.display='none';txt.style.display='none';out.style.display='none';nb.style.display='none';
  if(mode==='flag'||(mode==='capital'&&game.capitalDir==='flag2cap')){img.src='data/flags/w320/'+ISO2[id]+'.png';img.classList.remove('flag-img-wappen');img.style.display='';}
    ...
    const ns=NEIGHBOR_PUZZLE_MAP.get(id)||[];
    nb.innerHTML=ns.map(nid=>'<img src="'+'data/flags/w320/'+ISO2[nid]+'.png" alt="" style="width:110px;border-radius:6px;box-shadow:0 4px 24px rgba(0,0,0,0.5);border:1px solid #333;">').join('');
    nb.style.display='flex';
  }
  else{txt.textContent=game.capitalDir==='c2cap'?(C[id][lang]||C[id].de):(CAPITALS[id][lang]||CAPITALS[id].de);txt.classList.remove('iq-text-sentence');txt.style.display='';}
  $('flag-quiz-lbl').textContent=mode==='language'?t('iqAskLanguage'):mode==='currency'?t('iqAskCurrency'):mode==='neighbor'?t('iqAskNeighbor'):iqAnswerCapital()?t('iqAskCapital'):t('iqAskCountry');
...
function submitFlag(){
  ...
  // A correct answer schedules nextFlag() 500ms later (so the "✓" feedback is visible for a beat)
  // rather than advancing immediately — without this guard, a second submit for the same question
  // inside that window (e.g. Enter plus a dropdown click, or a fast double-Enter) would count twice
  // (game.flagCorrect++ / _challenge.firstTry++ both run again), inflating the score past the real
  // total. flagFound already tracks "this id is done" for every input mode, so reuse it as the guard.
  if(game.flagFound&&game.flagFound.has(id))return;
  ...
  if(val===norm(tg.de)||val===norm(tg.en)){
    ...
    game.flagWrong++;game.wrongOnCurrent=true;recordMissForCurrent();
    showFlagFb('✗ '+t('wrongShort'),'#D85A30');
    $('flag-input').value='';
    updateFlagStats();
  }
}
...
function updateFlagStats(){
  if(game.challengeMode){
    const g=_challenge;
    $('flag-skip-btn').style.display='';
    $('flag-score').textContent=g.pos+'/'+g.total;
    $('flag-stat-c').textContent=g.firstTry;
    $('flag-stat-w').textContent=game.flagWrong||0;
    if(g.daily){$('flag-stat-s').textContent=_formatDailyTime(Date.now()-g.startTs);$('flag-lbl-s').textContent=t('timeLbl');}
    else{$('flag-stat-s').textContent=g.skipped||0;$('flag-lbl-s').textContent=t('skippedLbl');}
    $('flag-stat-r').textContent=(g.total-g.pos)+' '+t('remLbl');
    $('flag-prog').style.width=Math.round((g.pos/g.total)*100)+'%';
    return;
  }
...
const REGION_QUIZZES={
  DE:{
    ...
    sub:{de:'Bundesländer',en:'Federal States',es:'Estados federados',fr:'Länder fédéraux'},
  ...
  US:{
    ...
    sub:{de:'Bundesstaaten',en:'States',es:'Estados',fr:'États'},
  ...
  FR:{
    name:{de:'Frankreich',en:'France',es:'Francia',fr:'France'},
    sub:{de:'Regionen',en:'Regions',es:'Regiones',fr:'Régions'},
  ...
  IT:{
    name:{de:'Italien',en:'Italy',es:'Italia',fr:'Italie'},
    sub:{de:'Regionen',en:'Regions',es:'Regiones',fr:'Régions'},
  ...
  ES:{
    ...
    sub:{de:'Autonome Gemeinschaften',en:'Autonomous Communities',es:'Comunidades autónomas',fr:'Communautés autonomes'},
  ...
  AT:{
    name:{de:'Österreich',en:'Austria',es:'Austria',fr:'Autriche'},
    sub:{de:'Bundesländer',en:'Federal States',es:'Estados federados',fr:'Länder fédéraux'},
  ...
  JP:{
    name:{de:'Japan',en:'Japan',es:'Japón',fr:'Japon'},
    sub:{de:'Präfekturen',en:'Prefectures',es:'Prefecturas',fr:'Préfectures'},
